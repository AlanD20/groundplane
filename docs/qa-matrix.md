...
| BACK-11 | Create Custom from an operator image; replay creation and reject image omission or managed-image override. | Atomic linked hierarchy with exact image and network, no invented Volume, credentials, healthcheck or ports; replay preserves identity. | U |
...
| BACK-13 | Fail, interrupt, time out, or return malformed/oversized/duplicate/undeclared hook facts; reconnect and explicitly retry. | No partial fact publication, leaked plaintext or silent command re-execution; Task reports failure and temporary results are cleaned up. | PARTIAL: parser rejection, sensitivity/size bounds, cancellation joining, uncertain-completion cleanup ordering and confidential errors pass race-enabled local checks. Live process termination and durable retry remain unrun. |
| BACK-14 | Start, stop, restart, and reboot a Custom backing with lifecycle hooks. | GP-requested before-stop/after-start ordering is respected; failed hooks block their operation; host/container autonomous restarts do not invoke hooks or replace Attach credentials. | U |
| BACK-15 | Create a custom hook owner in Blueprint; reference custom-hook facts and reuse an owner after standalone Attach completion. | New hook-owner creation rejects before publication with standalone-first guidance; completed Attach facts and owner reuse work normally. | U |
