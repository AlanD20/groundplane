...
| BP-02 | Validate a proposed create/update/removal against existing resources. | Accurate create/update/remove/retain diff and zero desired, Task, runtime or data writes. | U |
...
| BP-06 | Omit existing operator-managed resources from a new input; separately omit a persistent Volume. | Supported omissions remove exactly the declared resource with dependency-safe cleanup; Volume omission is rejected until protected Remove completes; generated Component state is not treated as an authored Entry. | U |
