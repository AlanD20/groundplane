...
| PKG-02 | Fresh install provisions prerequisites and enrolls the Agent; an existing guarded host uses native update only. Stage-only never activates or replaces the CLI. A successful automatic Controller update installs the matching CLI; a failed or uncertain Controller Task leaves the old CLI and recovery evidence. CLI activation failure reports incomplete installation for explicit retry. Config, keys, Runner, etcd and applications remain intact. | H74: successful native update from pinned old ref to `2e9a0319799` on disposable Ubuntu 24.04 amd64 PASS for matching installed CLI, healthy Controller/Agent, unchanged etcd/application identities and data under live traffic. Local behavioral checks cover failed/uncertain Task and CLI activation branches; those fault paths, other OS/architecture variants, and Runner/config/key contents were not live-qualified. |
...
| PKG-04 | Provision Ubuntu 24.04/26.04 and Debian 13 using their correct packages; refuse other distributions and ambiguous Docker installations. Repeated setup reuses running resources. Reinstalling the exact healthy release skips activation; interrupted updates retain the original Task. Unhealthy, foreign or incomplete state is not called success. | H69: five distro and eight idempotency tests pass; full 99-test deployment gate passes. Package/runtime effects are mocked; real-host OS/architecture matrix unrun. |
| PKG-05 | A tagged release runs required CI before publishing; builds native amd64/arm64 images and bundles; verifies each index contains precisely the tested platform digests; requires anonymous image access before publishing complete installer assets. | Three `scripts/test_release_indexes.py` checks pass for native pairs and platform/digest rejection; Actionlint passes. Full local CI is blocked by 122 existing architecture findings in `.tmp/release-0.0.1-ci-pinned.log`. GitHub execution, public downloads and actual registry publication remain unrun. |
| PKG-06 | With no version, the installer resolves one published stable release and pins both bundle and checksum URLs to it. Explicit versions bypass latest lookup; local bundles require a version. Missing releases, failed lookup, foreign redirects and non-stable tags stop before bundle download. Piped sh/bash input works. | `scripts/test_install_version.py`: host-free resolver/download boundary checks; live install remains unrun. |
...
| OWN-05 | Delete an owned populated aggregate through its confirmation workflow, including Services with no secondary indexes. | Descendants finalize in defined order and parent last; only selected resources/data are removed; unrelated applications still work. | PARTIAL (2026-09-20): strict-store Service finalization regression and sparse Environment cleanup after Retry passed on the previous QA installation. Project cleanup failed with conflicting deletion evidence. That installation was subsequently reset; fresh-host aggregate cleanup remains unqualified. The OWN-07 admission guard does not repair old plans. |
...
| OWN-07 | Delete a parent containing separately active work, including an already deleting child and a child deletion starting during parent publication. | Reject overlapping deletion without publishing the parent Task/tombstone or losing child authority; allow deletion when descendants are available. | PARTIAL (2026-09-20): `TestHierarchyDeletionRejectsDescendantDeletionAuthority` passed with `-race -count=1`: seven repository cases covering Project/Environment descendants, Project/Tenant parents, concurrent publication and available descendants. Live API/operator qualification and other active-work variants remain unrun. |
...
| Case | Setup and action | Pass condition | Record |
| --- | --- | --- | --- |
| UI-01 | Perform the same supported action/read through each operator surface. | Same intent, validation, resource/Task identity and observable effect; no missing Console action or frontend-only product decision. | PARTIAL H70: Console Blueprint Validate/review, API read and CLI mutations only; no Console Apply dispatch |
...
| Case | Setup and action | Pass condition | Record |
| --- | --- | --- | --- |
| BP-01 | Export, import, validate and export a complete Environment with every supported resource kind. | Canonical decisions round-trip, including Script bodies, exposure, grants and routing; no credentials, generated runtime, observations or lost required fields. | PARTIAL H70: Entry and direct Volume export/Validate only; complete resource set NOT RUN |
| BP-02 | Validate a proposed create/update/removal against existing resources. | Accurate create/update/remove/retain diff and zero desired, Task, runtime or data writes. | PARTIAL H70; FAIL H71 implicit default Zone accepted; H72 closed that admission variant. H73 confirms unchanged Compose/Service/Zone still preview as update despite no runtime churn. |
...
| BP-04 | First Apply a valid complete workload with host-local images and ordered setup hooks. | Declared resources and exact replicas serve real requests; all selected pre-hooks clean up before any candidate consumer starts. | PARTIAL H3; H72 one healthy synthetic Service without hooks or request traffic |
| BP-05 | Reapply the exact successful Blueprint. | Same stable IDs, generated values and serving Releases; no duplicate resources, restarted workloads or rerun hooks. | PASS H35 first-cycle router/native/held-HTTP variant; prior FAIL H29/H33. H70 retained Entry IDs/values without a workload. H73 retained one running Service, serving Release and container across 150 HTTP requests with no failures; no hook or proxy variant. |
| BP-06 | Omit existing operator-managed resources from a new input; separately omit a persistent Volume. | Supported omissions remove exactly the declared resource with dependency-safe cleanup; Volume omission is rejected until protected Remove completes; generated Component state is not treated as an authored Entry. | PARTIAL H70/H72: Volume refusal and one running-Service Entry omission passed; other resources/recovery NOT RUN |
...
| BP-09 | Export direct and Blueprint-created Entries, including secret literals; Apply unchanged and changed keys; omit one key. | Direct Entries retain ID and selected generation on adoption; secret values never enter Blueprint input/export; new key-only secret value is empty; omission removes only its selected Entry; identity changes follow the documented replacement rule. | PARTIAL H70/H72: adoption, empty secret, rotation, omission and selected running-file/next-Deploy timing; identity replacement NOT RUN |
...
| SVC-04 | Remove a Service using fixed-revision impact confirmation, including a directly created Service with no runtime files. | Referencing resources handled as declared; the removal candidate preserves absent optional files; only selected runtime/definition removed; unrelated Volumes, Services and Release history preserved. | PARTIAL (2026-09-20): direct creation → removal candidate → durable intent regression passed on the earlier QA repair source. This is not live removal proof; fresh-host removal and remaining cases remain unqualified. |
...
| SVC-09 | Fail a candidate before health/promotion with a real serving predecessor; include a created, unhealthy candidate beside the old serving slot and a post-deploy hook failure after candidate creation. | Exact predecessor remains/restores, independent application check succeeds, original failure persists on the same Task; no migration reversal. Recovery recognizes only the sealed candidate, rejects wrong-plan/release/image and unknown containers, and never counts the candidate as healthy predecessor evidence. | PARTIAL H66/H67: local identity/health negatives pass; live hook-failure recovery, preserved application/runtime, Task closure and subsequent Deploy pass. Candidate-health timeout on the corrected build is not rerun. |
...
| LOG-01 | Read finite and follow logs for Service and Environment, including replicas and stopped workloads. | Correct selected sources and tail bounds; no unrelated scope or retained inactive-slot output. | PARTIAL 2026-09-20: real single-Service finite tail10 and bounded follow pass for Service/Environment; correct source/release, stdout/stderr and sequence. Multi-Service, replicas and stopped workloads unrun. Evidence: `.tmp/qa-log-repair-bnmFip/log-qa-result.md`. |
| LOG-02 | Disconnect/reconnect, cancel or stall log streams. | Transient stream semantics are explicit; no invented historical resume, leaked subscription or durable Task-output storage. | FAIL/PARTIAL 2026-09-20: Console Service open/stop/reopen works; Environment first follow stream unexpectedly closed at 23 events. The Agent's fail-fast 128-record queue could cancel a normal tail200 burst and discard its queued remainder; local correction now backpressures the bounded queue and proves 200-record replay, continued follow and cancellation cleanup. QA rerun remains pending. Evidence: `.tmp/qa-log-repair-bnmFip/log-qa-result.md`; `TestLogManagerBackpressuresReplayLargerThanQueueUntilCancellation`. |
...
| VOL-07 | Fail, Abort, time out or restart during Volume removal, then use allowed Retry. | Exact ownership/removal authority survives; reconcile proven effects without losing retry state or reporting partial cleanup as success. | PARTIAL H26: local successor completion, lost-response replay and stale-Task refusal pass after the publisher correction. Live variants NOT RUN. |
...
| ENT-01 | Create plain, empty-secret and nonempty-secret Entries; inspect reads/history plus controlled storage/materialization. | Only documented plain values are public; secret values remain encrypted at rest and confined to authorized reveal/materialization; list/detail report only an accurate empty-value indicator for the selected secret generation. | PARTIAL H70: selected-value API/CLI/Console reads and explicit Reveal; storage/materialization NOT RUN |
...
| BACK-11 | Create Custom from an operator image; replay creation and reject image omission or managed-image override. | Atomic linked hierarchy with exact image and network, no invented Volume, credentials, healthcheck or ports; replay preserves identity. | PARTIAL: race-enabled application checks prove network-only publication and parsed runtime image/defaults plus input admission. Live runtime and replay qualification pending. |
| BACK-12 | Run attach/detach hooks with consumer inputs and declared ordinary/secret facts; include empty values and embedded equals signs. | Exact stable context and literal result values reach only the intended Attach; encrypted facts publish only after success and are available to detach. | PARTIAL: common result parser preserves literal/empty values and declared sensitivity; runtime and encrypted publication unrun. |
| BACK-13 | Fail, interrupt, time out, or return malformed/oversized/duplicate/undeclared hook facts; lose an acknowledgement, reconnect and explicitly retry; edit/delete a pinned Secret and expire retained inputs. | No partial facts, leaked plaintext or silent command re-execution; duplicate results preserve the first record. Retry uses captured inputs; Secret deletion blocks while pinned, and success/expiry removes retained source bytes. | PARTIAL: parser bounds, executor cancellation/uncertain cleanup, confidential errors and lost-acknowledgement repository replay pass race-enabled checks. Live termination/retry and hook-specific Secret release/expiry remain unrun. |
| BACK-14 | Create, start, stop, restart, and reboot a Custom backing with lifecycle hooks. | GP-requested before-stop/after-start ordering is respected, including initial creation; failed hooks block their operation; autonomous restarts do not invoke hooks or replace Attach credentials. | PARTIAL: initial creation plan puts after-start after Compose; terminal completion without its RESULT is rejected and failure remains Failed. Race-enabled local checks pass. Explicit runtime lifecycle/reboot variants remain unrun. |
| BACK-15 | Create a custom hook owner in Blueprint; reference custom-hook facts and reuse an owner after standalone Attach completion. | New hook-owner creation rejects before publication with standalone-first guidance; completed Attach facts and owner reuse work normally. | PARTIAL: race-enabled admission check rejects before candidate allocation; retained-owner preparation passes. Actual hook-fact consumption remains unrun. |
...
| ATT-13 | Attach and detach consumers of a Custom backing Service without hooks. | GP runs the selected image; attaching changes only the network edge, with no generated database, role, password, facts or GP backup. | U |
...
The remaining UP-03 candidate variants stage separate owned release fixtures with
an incompatible storage epoch, incompatible channel schema, or corrupted binary.
Only the new corrupt fixture's bytes may be changed, before any Task refers to
it; never alter a prior release or journal. Require 422 before Task publication,
...
| UP-06 | Abort after activation is committed or uncertain. | Refuse unsafe cancellation; preserve recovery and dispatch hold until exact commit outcome is resolved. | PARTIAL H56: committed `starting` Abort refused; uncertain publication remains local proof. |
...
| UP-08 | Fail candidate readiness or coordinated Agent startup after native activation. | Restore compatible predecessor binary/Agent identities, preserve selected qualified release and application state; recovery failure is explicit. | PASS H56 unready-Agent coordinated recovery, private ingress |
| UP-09 | Restart/kill Controller at each declared activation-journal phase, including lost commit response. | Durable original Task and holds restored before dispatch; finish or recover exact candidate once without reopening an obsolete generation. | PARTIAL H57: exact-inode kill at real `starting` recovers same Task and Agent; other journal phases/lost commit remain unqualified live. |
| UP-10 | Attempt ordinary writes and scheduled work during an unfinished native trial. | Guarded writes refuse; permitted reads, exact acceptance replay and pre-activation Abort still work; mere HTTP readiness grants no write authority. | PARTIAL H56: live Script edit/Run refusal, reads/replay and resumed writes pass; scheduled mutation isolation remains local proof. |
...
The owner-approved recreate Deploy passes independent runtime-policy and canary
checks in H64. After the owner restarts the killed host, the singleton workload
plus serving proxy recover without container-level intervention. Automatic host
return FAILS; the observed 220.84 seconds includes owner intervention. This does
not qualify the separate blue-green boot variant or close H63's limitation.
...
H66 fixes the candidate/predecessor observation mismatch with local regression
proof. H67 qualifies a new live hook-failure recovery and subsequent Deploy on the
corrected build; neither reclassifies or rewrites the original failed Task.
...
Apply [testing policy](delivery.md#testing-policy) first: straightforward
implementation does not need tests; uncertain cases require the owner's decision.
Each retained test must answer: which requirement or real failure does it guard,
...
Retain tests with a concrete complexity or edge-case reason; add missing rationale
and case links where needed. Remove tests of straightforward implementation,
...
Static source-text and contract-shape tests are prohibited. Finding a callback
name, UI string or declaration does not prove behavior. Deleting
one does not qualify its feature: keep the missing interaction/runtime proof
visible; any future test must first satisfy the testing policy. Do not add a new
...
| --- | --- | --- |
| Three former `scripts/test_build_pages.py` tests | Removed at the owner's request. Static page staging does not warrant a dedicated test suite. Removed the Pages workflow test invocation too. | No product hosting or recovery proof removed. |
| `TestOnFailureWithDefault` in `internal/core/release_group_test.go` | Removed. This tests a direct empty-value fallback and unchanged passthrough; no complex sequence or distinct edge case. Group constructor/default and policy-validation tests remain. | No coordinated rollout or recovery proof claimed by this helper test. |
...
| Three in [hook result behavior](../internal/common/backinghook/result_test.go) | Literal first-equals parsing, empty values, declared sensitivity, independent result buffers and all-or-nothing rejection of invalid output. Native output obeys the same size and sensitivity rules. | BACK-12/13: race-enabled local checks pass; no command execution, persistence or retry proof. |
| Two in [hook executor behavior](../internal/agent/backing_hook_test.go) | Cancellation and Docker transport failure cannot race result cleanup or publish facts; absolute Task budget and error confidentiality remain enforced. | BACK-13: race-enabled fake-Runner checks pass; real container TERM/KILL behavior unrun. |
| `TestBackingHookCheckpointReplaysResultWithoutReplacingCiphertext` in [hook checkpoints](../internal/infra/etcd/backing_hook_checkpoint_test.go) | A lost acknowledgement reuses the first encrypted result without another write; changed results and stale assignments cannot borrow its success. | BACK-13: focused race-enabled repository check passes; live retry remains unrun. |
| [Creation hook ordering](../internal/controller/backing_service_hook_order_test.go) and [hook terminal gating](../internal/infra/etcd/backing_hook_terminal_behavior_test.go) | Reconstructed creation puts after-start behind Compose; a missing RESULT cannot complete the Task, while hook failure stays Failed. | BACK-13/14: focused race-enabled checks pass; not live container or Secret-retention proof. |
...
| Two export-lifecycle tests in former `backup-key-export-contract.test.mjs` | Removed. Any matching no-store/finally/abort text could satisfy them without executing key export, revocation of the download URL or unmount cleanup. The remaining source-type spelling test and file were also removed under the testing policy. | BAK-14 and UI-05 require browser lifecycle/secret-safety proof; source spelling is not proof. |
...
| Three tests in [component-zone-picker.test.mjs](../console/src/lib/component-zone-picker.test.mjs) | Retained. Select the exact returned ID, leave no phantom selection on rejection, and exclude backing-owned options without guessing names/internal state. Removed the direct add/remove array-helper test as straightforward logic. | NET-02, CMP-01/03, HTTP-07, UI-03: local callbacks and selection only. |
...
| Former `problem_generated_contract_test.go` | Removed reflected field inventory and generated-source string matching. | Behavioral response limits and body-close tests remain. |
...
| Five in [test_deploy_images.py](../scripts/test_deploy_images.py) | Retained deployment behavior tests; removed the Dockerfile source-string assertion. | Command effects and failure branches, not image execution. |
...
| Three in [logs_test.go](../internal/agent/logs_test.go) | Typed open-error classification, zero-source readiness/completion, and cancellation-aware bounded backpressure that carries a 200-record replay into continued follow before cleanup. | LOG-01/02, TASK-07: injected sources and local queues, not Docker source selection, public SSE or client reconnect. |
...
| Four in [task_event_routes_test.go](../internal/controller/task_event_routes_test.go) | Retained resume parsing, SSE forwarding and pre/post-header failures; removed OpenAPI structure assertions. | TASK-06/07, UI-03: local HTTP boundary. |
| Nine in [task_read_routes_test.go](../internal/controller/task_read_routes_test.go) | Retained projection, cursor and scope behavior; removed two OpenAPI identity/enum checks. | TASK-01/06, BAK-08, UI-03: local boundaries. |
