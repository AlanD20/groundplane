...
| BP-02 | Validate a proposed create/update/removal against existing resources. | Accurate create/update/remove/retain diff and zero desired, Task, runtime or data writes. | PARTIAL H70; FAIL H71 implicit default Zone accepted; H72 closed that admission variant. Unchanged Service/Zone still preview as update. |
...
| BP-05 | Reapply the exact successful Blueprint. | Same stable IDs, generated values and serving Releases; no duplicate resources, restarted workloads or rerun hooks. | PASS H35 first-cycle router/native/held-HTTP variant; prior FAIL H29/H33; H70 Entry IDs/values held, no serving workload |
