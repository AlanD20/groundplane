...
| BP-02 | Validate a proposed create/update/removal against existing resources. | Accurate create/update/remove/retain diff and zero desired, Task, runtime or data writes. | PARTIAL H70: selected Entry create/remove and Volume omission refusal; unchanged resources reported update. FAIL H71: Validate accepted an implicit default Zone that Apply rejected. |
...
| BP-06 | Omit existing operator-managed resources from a new input; separately omit a persistent Volume. | Supported omissions remove exactly the declared resource with dependency-safe cleanup; Volume omission is rejected until protected Remove completes; generated Component state is not treated as an authored Entry. | PARTIAL H70: isolated Entry omission and Volume refusal only; other resources/runtime NOT RUN |
