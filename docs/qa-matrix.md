...
| BACK-13 | Fail, interrupt, time out, or return malformed/oversized/duplicate/undeclared hook facts; reconnect and explicitly retry. | No partial fact publication, leaked plaintext or silent command re-execution; Task reports failure and temporary results are cleaned up. | PARTIAL: parser rejects malformed, duplicate, undeclared, missing, NUL and oversized output without partial facts; native output cannot downgrade sensitivity. Runtime failure/retry unrun. |
