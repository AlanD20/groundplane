...
| SVC-15 | Change Attach and Entry configuration successfully, then fail the next rollout, including unrelated Volumes in the shared project. | Actual predecessor uses the last successfully applied bindings and serves authenticated requests; unrelated named Volumes cannot block proof, required/unknown Volume collisions must block; original Release input/history is not rewritten. | BLOCKED H36; H37 local Volume-scope fix, journal saturation unresolved; prior FAIL H5 |
