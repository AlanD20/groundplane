...
| SVC-09 | Fail a candidate before health/promotion with a real serving predecessor; include a created, unhealthy candidate beside the old serving slot. | Exact predecessor remains/restores, independent application check succeeds, original failure persists on the same Task; no migration reversal. Recovery recognizes only the sealed candidate, rejects wrong-plan/release/image and unknown containers, and never counts the candidate as healthy predecessor evidence. | PARTIAL H66: local observer/Agent sequence and negative identity/health cases pass; corrected live failed-Deploy journey remains unrun. |
...
proof; it does not reclassify the original failed Task or qualify live recovery.
