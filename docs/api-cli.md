...
| backing-service | `GET /backing-services` and `GET /backing-services/{project_id}` → `200` · protected `POST /backing-services` body `{slug,name,description?,adapter:"postgres:16"\|"valkey:9",authentication:"username_password"\|"password"\|"none" (required for Valkey; omitted otherwise),network_pool,zone:{name,subnet,internal}}` → `201 {backing_service,task_id}` atomically creates the backing Project, `main` Environment, dedicated backing-owned Zone, adapter Service, adapter data Volume, and Agent Task; `authentication` has no default and requires an explicit Valkey choice; missing, empty, null or unknown modes return `validation.failed` (422) before publication; the selected mode is immutable and returned by the facade; PostgreSQL rejects this field; the Zone subnet must be inside the Environment pool and globally unreserved · bodyless `POST /backing-services/{project_id}/start\|stop\|destroy` → `202 {task_id}` and delegates to the facade's immutable adapter Service; there is no Backing-service delete endpoint, existing-Zone create branch, uploaded Blueprint, or omitted subnet default |
