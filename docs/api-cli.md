...
Command tree:
    ...
    │                      --adapter postgres:16|valkey:9|custom [--image <image>]
    │                      --authentication username_password|password|none (required for valkey:9; omitted otherwise)
...
The [Backup contract](features/backups.md) adds no operator action or alternate wire
...
Custom creation accepts `image` and optional `hooks`; managed adapters reject
both fields. The CLI exposes these as `backing-service create --adapter custom
--image IMAGE --hooks-file FILE`. The file contains a JSON hook configuration;
`-` reads stdin. The Console presents the same optional commands, timeouts,
inputs and fact declarations in the creation form.

The ordinary Service read includes `hooks` for a configured custom backing
Service. `PATCH /services/{id}` replaces it only when `hooks` is supplied;
omission preserves the current configuration and `{}` clears it. The matching
actions are `service edit --hooks-file FILE` and Edit service in the Console.
Changes affect newly captured operations, never previously accepted Tasks.
Create a custom hook credential owner through the existing Attach action, wait
until it is ready, then use its facts or reuse that owner from Blueprint.
Blueprint creation of new custom hook owners is deferred and rejects before
publication; it does not silently skip provisioning.
The shape and execution rules are defined in
[Backing services](features/backing-services.md#unified-provisioning-and-optional-custom-hooks).
...
| backing-service | `GET /backing-services` and `GET /backing-services/{project_id}` → `200` · protected `POST /backing-services` body `{slug,name,description?,adapter:"postgres:16"\|"valkey:9"\|"custom",image? (required for Custom; rejected otherwise),authentication:"username_password"\|"password"\|"none" (required for Valkey; omitted otherwise),network_pool,zone:{name,subnet,internal}}` → `201 {backing_service,task_id}` atomically creates the backing Project, `main` Environment, dedicated backing-owned Zone, adapter Service, adapter data Volume for built-in database adapters, and Agent Task; Custom runs the required operator image without invented storage, credentials, ports or healthcheck; `authentication` has no default and requires an explicit Valkey choice; missing, empty, null or unknown modes return `validation.failed` (422) before publication; the selected mode is immutable and returned by the facade; PostgreSQL rejects this field; the Zone subnet must be inside the Environment pool and globally unreserved · bodyless `POST /backing-services/{project_id}/start\|stop\|destroy` → `202 {task_id}` and delegates to the facade's immutable adapter Service; there is no Backing-service delete endpoint, existing-Zone create branch, uploaded Blueprint, or omitted subnet default |
