...
Command tree:
    ...
    │                    rename | delete
    │                    apply <slug> --bundle-dir DIR --root RELATIVE_PATH
...
- **The only nesting is singleton sub-resources** — resources that are
  ...
  (`/environments/{id}/blueprint`) is the write-only desired-state replacement
  singleton. That is the standard singleton pattern, not hierarchy nesting.
