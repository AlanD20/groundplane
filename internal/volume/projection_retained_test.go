...
// Rationale: removing a Volume must not turn an independently serving proxy and
// one retained slot into a fresh, incomplete blue/green deployment.
func TestVolumeRemovalProjectionPreservesRetainedRuntime(t *testing.T) {
	for _, slot := range []domain.WorkloadTarget{domain.WorkloadBlue, domain.WorkloadGreen} {
		t.Run(string(slot), func(t *testing.T) {
			...
			if !bytes.Equal(cleanup.CanonicalYaml, artifact.CanonicalYaml) ||
				...
				strings.Contains(string(next.CanonicalYaml), "scratch") ||
