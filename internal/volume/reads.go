...
func (service *ReadService) GetVolumeDeletionImpact(
	...
	if encodedCursor == "" {
		...
		cursor = decoded
		projection, _, err = service.repository.GetEnvironmentComposeProjectionRevision(
			ctx, cursor.EnvironmentID, cursor.RevisionID,
		)
		if err != nil {
			return apiTypes.VolumeDeletionImpactPage{}, err
		}
		...
		if identity.ID == "" {
			return apiTypes.VolumeDeletionImpactPage{}, errs.New(errs.KindStateConflict, "Volume impact revision is corrupt")
		}
	}
	items := volumeMountImpactItems(projection.Record, volumeID)
	...
	rolling, err := volumeImpactDigest(projection.Record, identity, items[:end])
	if err != nil {
		return apiTypes.VolumeDeletionImpactPage{}, err
	}
	...
	if page.Complete {
		page.ImpactToken, err = volumeImpactDigest(projection.Record, identity, items)
		if err != nil {
			return apiTypes.VolumeDeletionImpactPage{}, err
		}
	} else {
...
func volumeImpactDigest(
	...
	}{projection.EnvironmentID, projection.RevisionID, projection.RenderGeneration, identity, items})
	if err != nil {
