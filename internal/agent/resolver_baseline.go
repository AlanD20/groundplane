...
func CaptureHostResolverBaseline(
	ctx context.Context,
	path string,
	generation uint64,
) (coredns.ResolverBaseline, error) {
	if ctx == nil {
	...
	if generation == 0 || !filepath.IsAbs(path) || filepath.Clean(path) != path {
		return coredns.ResolverBaseline{}, errs.New(
			errs.KindValidationFailed,
			"coredns: resolver baseline path or generation is invalid",
		)
	}
