...
func (fixture *ExecutedArtifactFixture) PublishManualJourneyEntry(
	...
	ciphertext []byte,
	secretID string,
) (*testentryvalues.Repository, *etcd.SecretRepository, testentries.Record) {
...
func (fixture *ExecutedArtifactFixture) publishJourneyEntry(
	...
	secrets, err := etcd.NewSecretRepository(fixture.store)
	if err != nil {
		t.Fatal(err)
	}
	...
	entries, err := etcd.NewEntryRepository(fixture.store)
	if err != nil {
		t.Fatal(err)
	}
	...
	fixture.Publish(t, task, projection, etcd.BlueprintReleasePublication{})
