...
type attachFactReadRecordStub struct {
	record etcd.Versioned[etcd.AttachRecord]
}
...
func TestAttachFactReadUsesDurableEnvironmentScope(t *testing.T) {
	...
	service, err := newAttachFactReadService(attachFactReadRecordStub{record: etcd.Versioned[etcd.AttachRecord]{
	...
	if err != nil {
		t.Fatalf("newAttachFactReadService() error = %v", err)
	}
