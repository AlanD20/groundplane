...
func TestCustomBackingServiceCreationPublishesNetworkOnlyRuntime(t *testing.T) {
	...
	service, err := newBackingServiceCreationService(
		volumeRoot,
	...
	if creation.Task.Params[etcd.TaskBackingServiceCreationParam] != desired.ID ||
		creation.Task.Params[etcd.TaskBackingServiceHealthParam] != "" ||
		creation.Task.Params[controller.EnvironmentBlueprintManagedVolumesParam] != "" ||
		len(creation.Task.Materializations) != 0 || len(creation.Task.Steps) != 2 {
		t.Fatalf("custom creation Task = %#v", creation.Task)
	}

	resolver, err := controller.NewTaskPlanResolverWithBlueprints(volumeRoot, repository, nil)
	if err != nil {
		t.Fatal(err)
	}
