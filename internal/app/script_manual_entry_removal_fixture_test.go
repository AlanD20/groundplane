...
func (fixture *ExecutedArtifactFixture) EntryRemovalTask(
	t *testing.T,
	entryID string,
) (etcd.TaskRecord, testidempotency.IdempotencyMarker) {
	t.Helper()
...
func (fixture *ExecutedArtifactFixture) CheckManualJourneyEntryRemoval(t *testing.T, entryID string, active bool) {
	...
	result, err := fixture.tryPublish(t, task, projection, etcd.BlueprintReleasePublication{})
	if active {
		if !isKind(err, errs.KindResourceInUse) &&
			!isKind(integrationTransactionConflict(result), errs.KindResourceInUse) {
			t.Fatalf(
				"desired Entry removal ignored retained Script generation: %v / %v",
				err,
				integrationTransactionConflict(result),
			)
		}
	...
	if err != nil || !integrationTransactionApplied(result) {
		t.Fatalf("unblocked Entry removal = %v / %v", err, integrationTransactionConflict(result))
	}
	fixture.head = fixture.store.revision
