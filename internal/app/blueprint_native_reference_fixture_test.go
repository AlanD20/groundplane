...
func (fixture *ExecutedArtifactFixture) AssertBlueprintNativeReferenceRejection(t *testing.T, task etcd.TaskRecord) {
	t.Helper()
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			tasks, err := etcd.NewTaskRepository(fault)
			if err != nil {
				t.Fatal(err)
			}
			ledger, err := etcd.NewReleaseLedger(fault, tasks)
			if err != nil {
				t.Fatal(err)
			}
			before := fixture.ReadRevision()
			if _, err := ledger.GetBlueprintTaskRenderInput(ctx, task); !errors.Is(
				err,
				errs.New(errs.KindInternal, ""),
			) {
				t.Fatalf("corrupt native source admitted for plan reconstruction: %v", err)
			}
			if _, found, err := tasks.ClaimNextTask(ctx, ids.New(ids.KindAgent), 1, task.CreatedAt.Add(time.Second)); !errors.Is(
				err,
				errs.New(errs.KindInternal, ""),
			) ||
				found {
...
func (fixture *ExecutedArtifactFixture) AssertBlueprintNativePublicationSourceFences(
	t *testing.T, task etcd.TaskRecord, _ etcd.BlueprintReleasePublication,
) {
	t.Helper()
	if fixture.publicationSize == nil {
		t.Fatal("native source-fence assertion requires publication auditing")
	}
	ctx := context.Background()
	...
	for _, member := range manifest.Members {
		...
		if err != nil || read.Entry == nil {
			t.Fatalf("staged native bytes are unavailable to public reconstruction: %v", err)
		}
		fixture.publicationSize.SourceFences[key] = read.Entry.ModRevision
		...
		if err != nil || runtimeRead.Entry == nil {
			t.Fatal("acknowledged native runtime is unavailable to public reconstruction")
		}
		fixture.publicationSize.SourceFences[runtimeKey] = runtimeRead.Entry.ModRevision
	}
}
