...
func (fixture *ExecutedArtifactFixture) CreateManualJourneySecret(t *testing.T, ciphertext []byte) string {
	...
	repository, err := etcd.NewSecretRepository(fixture.store)
	if err != nil {
		t.Fatal(err)
	}
...
func (fixture *ExecutedArtifactFixture) CheckManualJourneySecretDeletion(t *testing.T, secretID string, active bool) {
	...
	repository, err := etcd.NewSecretRepository(&connectorReferenceRaceStore{
	...
	if active {
		...
		if err != nil || !isKind(integrationTransactionConflict(result), errs.KindResourceInUse) ||
			fixture.store.revision != before {
			t.Fatalf(
				"active manual Secret deletion should reject without writes: %v / %v",
				err,
