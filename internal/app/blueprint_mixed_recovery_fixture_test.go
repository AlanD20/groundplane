...
func (fixture *ExecutedArtifactFixture) ProveMixedMemberRecovery(t *testing.T, agentID string,
	claim etcd.TaskAssignment, plan *agentpb.ExecutionPlan, servingServiceID, priorReleaseID string,
	runRecovery ...func(etcd.TaskAssignment, testtaskjournal.TaskResultRecord) testtaskjournal.TaskResultRecord,
) {
	t.Helper()
	...
	if _, err := reopened.AcknowledgeTask(ctx, agentID, 1, task.ID, assignment.AssignmentID, testtaskjournal.TaskStatusFailed, changedPrimary, task.CreatedAt.Add(4*time.Second)); !errors.Is(
		err,
		errs.New(errs.KindStateConflict, ""),
	) {
		t.Fatalf("changed old-epoch report accepted: %v", err)
	}
	planSteps := make(map[string]struct{}, len(plan.GetSteps()))
	for _, step := range plan.GetSteps() {
		planSteps[step.GetStepId()] = struct{}{}
	}
	seenRecoverySteps := make(map[string]struct{}, len(recovery.ReleaseRecovery.StepIDs))
	for _, stepID := range recovery.ReleaseRecovery.StepIDs {
		if _, ok := planSteps[stepID]; !ok {
			t.Fatalf("mixed recovery selected a step outside the sealed plan: %s", stepID)
		}
		if _, duplicate := seenRecoverySteps[stepID]; duplicate {
			t.Fatalf("mixed recovery repeated sealed step: %s", stepID)
		}
		seenRecoverySteps[stepID] = struct{}{}
	}
	if len(seenRecoverySteps) == 0 {
		t.Fatal("mixed recovery omitted all sealed recovery steps")
	}
	...
	var terminal testkeyvalue.Versioned[etcd.TaskRecord]
	...
	if _, err := reopened.AcknowledgeTask(ctx, agentID, 1, task.ID, assignment.AssignmentID, testtaskjournal.TaskStatusCompleted, *changed, task.CreatedAt.Add(33*time.Second)); !errors.Is(
		err,
		errs.New(errs.KindStateConflict, ""),
	) {
...
func mixedRecoveryResult(t *testing.T, recovery etcd.TaskAssignment, procedure *agentpb.CandidateReleaseProcedure,
