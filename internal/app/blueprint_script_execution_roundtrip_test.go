...
func TestGetBlueprintScriptExecutionReconcilesWithoutChangingGeneration(t *testing.T) {
	...
	service, err := etcd.NewServiceRecord(
		environmentID,
	...
	script, err := etcd.NewScriptRecord(environmentID, serviceID, core.Script{ID: ids.NewAt(ids.KindScript, at, 8),
	...
	repository := &authoringRoundtripRepository{
		...
		projection: etcd.EnvironmentComposeProjection{EnvironmentID: environmentID, RevisionID: revisionID,
			...
			Volumes: []etcd.EnvironmentVolumeIdentity{{ID: volumeID, Key: "data", Slug: "renamed-data"}},
		...
		scripts: []etcd.ScriptRecord{script},
	}
	blueprints := &environmentBlueprintService{repository: repository, releaseGroups: planner}
	...
	reconciled, err := desiredrevision.ReconcileBlueprintScripts(environmentID, parsed.Extensions.Scripts,
		[]etcd.ServiceRecord{service}, []etcd.ScriptRecord{script}, desiredrevision.BlueprintScriptResources{
