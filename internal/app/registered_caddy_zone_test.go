...
func TestProjectRegisteredCaddyInputPreservesOrderedZones(t *testing.T) {
	...
	input, _, err := projectRegisteredCaddyInput(environment, caddy)
	if err != nil {
		t.Fatalf("projectRegisteredCaddyInput() error = %v", err)
	}
	if len(input.Zones) != 2 || input.Zones[0].ID != primaryID || input.Zones[0].Name != "primary" ||
		...
		t.Fatalf("projectRegisteredCaddyInput() Zones = %#v", input.Zones)
	}
}
