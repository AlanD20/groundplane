...
func TestAttachProcedureTransportsPasswordAuthenticationAndDetachSecret(t *testing.T) {
	...
	for _, taskType := range []etcd.TaskType{etcd.TaskAttach, etcd.TaskDetach} {
...
func TestAttachProcedureEmitsNoEffectForNoAuthentication(t *testing.T) {
	...
	steps, err := BuildAttachProvisionSteps(
		etcd.TaskRecord{Type: etcd.TaskAttach, TimeoutSeconds: 30},
		etcd.AttachRecord{ID: ids.New(ids.KindAttach), BackingServiceID: ids.New(ids.KindService)},
		"valkey:9",
...
func TestBlueprintAttachPlanReplaysNoAuthenticationWithoutProcedure(t *testing.T) {
	...
	record, err := etcd.NewPendingAttachRecord(
		...
		[]etcd.AttachFactSetMetadata{{Facts: []etcd.AttachFactDefinition{{Key: "valkey9_HOST"}}}},
		task.ID, now,
	)
	if err != nil {
	...
	intent := etcd.BlueprintAttachTaskIntent{
		TaskID: task.ID, EnvironmentID: reader.environment.ID, Status: etcd.TaskStatusPending,
