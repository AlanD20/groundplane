...
func TestBuildAttachTaskRenderInputSortsRunningIdentityCopy(t *testing.T) {
	...
	input, err := buildAttachTaskRenderInput(
		fixture.scope,
		testtaskplanning.EntryMutationRuntime{Projection: fixture.scope.ComposeProjection.Record,
...
func TestBuildAttachTaskRenderInputCreatesCompleteNetworkUnion(t *testing.T) {
	...
	detaching, err := testattachments.MarkAttachProvisioning(detaching, detaching.TaskID)
	if err != nil {
		t.Fatalf("MarkAttachProvisioning() error = %v", err)
	}
	detaching, err = testattachments.CompleteAttachProvisioning(detaching, detaching.TaskID, true)
	if err != nil {
		t.Fatalf("CompleteAttachProvisioning() error = %v", err)
	}
	detaching, err = testattachments.BeginAttachDetaching(detaching, ids.NewAt(ids.KindTask, fixture.now, 132))
	if err != nil {
	...
	input, err := buildAttachTaskRenderInput(
		fixture.scope,
		testtaskplanning.EntryMutationRuntime{Projection: fixture.scope.ComposeProjection.Record, EpochRevision: 1},
		fixture.current,
...
func TestBuildAttachTaskRenderInputDetachesOnlyTargetMembership(t *testing.T) {
	t.Parallel()
	fixture := newAttachRenderFixture(t)
	ready, err := testattachments.MarkAttachProvisioning(fixture.current, fixture.current.TaskID)
	if err != nil {
		t.Fatalf("MarkAttachProvisioning() error = %v", err)
	}
	ready, err = testattachments.CompleteAttachProvisioning(ready, ready.TaskID, true)
	if err != nil {
		t.Fatalf("CompleteAttachProvisioning() error = %v", err)
	}
	detachTask := fixture.createTask
	detachTask.ID = ids.NewAt(ids.KindTask, fixture.now, 50)
	detachTask.Type = testtaskjournal.TaskDetach
	detachTask.PlanID = ids.NewAt(ids.KindPlan, fixture.now, 51)
	detaching, err := testattachments.BeginAttachDetaching(ready, detachTask.ID)
	if err != nil {
		t.Fatalf("BeginAttachDetaching() error = %v", err)
	}
...
type attachRenderFixture struct {
	...
	current          testattachments.Record
...
func newAttachRenderFixture(t *testing.T) attachRenderFixture {
	...
	fixture.scope = etcd.AttachCreateScope{
		Tenant: testkeyvalue.Versioned[testhierarchy.TenantRecord]{
			Record: testhierarchy.TenantRecord{ID: fixture.tenantID, Slug: "acme"}, Revision: 1,
		},
		Project: testkeyvalue.Versioned[testhierarchy.ProjectRecord]{
			Record: testhierarchy.ProjectRecord{
				ID: fixture.projectID, TenantID: fixture.tenantID, Slug: "shop", Kind: testhierarchy.ProjectKindTenant,
			},
			Revision: 2,
		},
		...
		ComposeProjection: testkeyvalue.Versioned[testenvironmentprojection.EnvironmentComposeProjection]{
			Record: testenvironmentprojection.EnvironmentComposeProjection{
				...
				Volumes: []testenvironmentprojection.EnvironmentVolumeIdentity{
					{ID: fixture.volumeID, Slug: "data", Key: "data"},
				},
		...
		Services: []testkeyvalue.Versioned[testservices.ServiceRecord]{
			{
	...
	current, err := testattachments.NewPendingAttachRecord(
	...
	fixture.createTask = etcd.TaskRecord{
		ID: taskID, PlanID: ids.NewAt(ids.KindPlan, now, 15), Type: testtaskjournal.TaskAttach, Target: attachID,
		RenderGeneration: 4,
	}
	return fixture
}
