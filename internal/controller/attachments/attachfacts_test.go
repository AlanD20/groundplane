...
func TestAttachFactServiceSealsExplicitValkeyAuthenticationIdentity(t *testing.T) {
	...
	service, err := NewFactService(&attachFactTestRepository{}, protector)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAttachFactServiceSealsAndResolvesGrantFacts(t *testing.T) {
	...
	repository := &attachFactTestRepository{
		records: make(map[string]testkeyvalue.Versioned[testattachments.Record]),
	}
	service, err := NewFactService(repository, protector)
	if err != nil {
		t.Fatalf("NewFactService() error = %v", err)
	}
	...
	pending, err := testattachments.NewPendingAttachRecord(
	...
	var taskIdentity testtaskplanning.AttachPlanIdentity
	err = service.ResolveTaskIdentity(
		ctx, testkeyvalue.Versioned[testattachments.Record]{Record: pending, Revision: 9, ReadRevision: 9}, taskID,
		func(identity testtaskplanning.AttachPlanIdentity) error {
			taskIdentity = testtaskplanning.AttachPlanIdentity{
				Database: identity.Database, Role: identity.Role,
				Password: append([]byte(nil), identity.Password...),
				Grants:   append([]testtaskplanning.AttachPlanGrantIdentity(nil), identity.Grants...),
			}
			return nil
		},
	)
	...
	grantMetadata := []testattachments.FactSetMetadata{{Facts: metadata[0].Facts}}
	...
	repository.records["api-db"] = testkeyvalue.Versioned[testattachments.Record]{
		Record:       owner,
		Revision:     11,
		ReadRevision: 11,
	}
	...
	repository.records["reporting-db"] = testkeyvalue.Versioned[testattachments.Record]{
		Record:       grant,
		Revision:     12,
		ReadRevision: 12,
	}
...
func TestAttachFactServiceRequiresReadyAttach(t *testing.T) {
	...
	record := readyAttachRecord(
		...
		nil,
		[]testattachments.FactSetMetadata{{Facts: []testattachments.FactDefinition{{Key: "test_DATABASE"}}}},
		now,
		7,
	)
