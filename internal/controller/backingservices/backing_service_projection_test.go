...
func TestBackingServiceCreationProjectionIncludesGeneratedTopology(t *testing.T) {
	...
	zone := testzones.Record{EnvironmentID: environmentID, Desired: core.Zone{
	...
	projection := buildBackingServiceCreationProjection(
		environmentID, revisionID, testcomposeidentity.Snapshot{
			Services: []testcomposeidentity.Resource{{ID: serviceID, Name: "postgres"}},
			Networks: []testcomposeidentity.Resource{{ID: networkID, Name: "data"}},
			Volumes:  []testcomposeidentity.Resource{{ID: volumeID, Name: "data"}},
