	...
	for _, input := range []struct {
		...
		t.Run(input.adapter+"/"+string(input.mode), func(t *testing.T) {
			...
			artifact, err := testcomposerender.RenderCompose(testcomposerender.ComposeRenderInput{
				Project: project, ProjectOwnerKind: testcomposerender.ComposeProjectOwnerBacking,
				...
				Identities: testcomposeidentity.Snapshot{
					Services: []testcomposeidentity.Resource{{ID: "svc_" + suffix, Name: spec.ServiceName}},
					Networks: []testcomposeidentity.Resource{{ID: zone.ID, Name: zone.Name}},
					Volumes:  []testcomposeidentity.Resource{{ID: volume.ID, Name: volume.Key}},
				},
			})
			if err != nil {
				t.Fatal(err)
