...
func TestOrdinaryReleasePreservesPriorConfiguration(t *testing.T) {
	...
	project, err := composerender.LoadNormalizedEnvironmentProject(t.Context(), member.Render.Projection)
	if err != nil {
		t.Fatal(err)
	}
	...
	member.Render.Projection.NormalizedCompose, err = composerender.MarshalNormalizedEnvironmentProject(project)
	if err != nil {
		t.Fatal(err)
	}
