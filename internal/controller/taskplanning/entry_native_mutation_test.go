...
func TestEntryMutationPreservesRetainedNativeOwnership(t *testing.T) {
	...
	entry := testentries.Record{EnvironmentID: current.EnvironmentID,
...
func testEntryMutationRuntimeSelection(t *testing.T, running bool) {
	...
	entry := testentries.Record{EnvironmentID: current.EnvironmentID,
		Entry: core.EnvEntry{ID: "ev_01ARZ3NDEKTSV4RRFFQ69G5FC1", Kind: core.EntryKindEnv,
			Key: "MODE", Source: core.EntrySource{Kind: core.SourceLiteral}, Exposure: []string{"api"}},
		CurrentValueGenerationID: "cfg_01ARZ3NDEKTSV4RRFFQ69G5FC2"}
	candidate, materials, err := composerender.ProjectEnvironmentEntryMutation(
		current,
		composerender.EnvironmentEntryArtifactMutation{
			RevisionID: "task_01ARZ3NDEKTSV4RRFFQ69G5FC3", ArtifactID: "cfg_01ARZ3NDEKTSV4RRFFQ69G5FC4",
			PlanID: "plan_01ARZ3NDEKTSV4RRFFQ69G5FC5", RenderGeneration: current.RenderGeneration + 1,
	...
	task := etcd.TaskRecord{ID: "task_01ARZ3NDEKTSV4RRFFQ69G5FC3", PlanID: "plan_01ARZ3NDEKTSV4RRFFQ69G5FC5",
		Type: testtaskjournal.TaskUpdate, Executor: testtaskjournal.TaskExecutorAgent, Target: current.EnvironmentID,
	...
	if running {
		...
		runtime.Sources = []testkeyvalue.Versioned[serviceruntimerecord.Record]{{
	...
	reader := &volumePlanReader{projections: map[string]testenvironmentprojection.EnvironmentComposeProjection{
	...
	foreign := task
	foreign.EntryRuntime = &testtaskjournal.EntryTaskRuntime{
		RunningServiceIDs: []string{ids.New(ids.KindService)}, Updates: []testtaskjournal.EntryRuntimeUpdate{},
	}
