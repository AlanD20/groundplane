...
func TestBuildPlanSealsDeterministicOwnedCopy(t *testing.T) {
	...
	first, err := testtaskplan.Build(input)
	if err != nil {
		t.Fatalf("BuildPlan() error = %v", err)
	}
	second, err := testtaskplan.Build(input)
	if err != nil {
...
func TestBuildPlanRejectsIncompleteTypedInput(t *testing.T) {
	...
	if _, err := testtaskplan.Build(input); err == nil {
...
func TestBuildPlanRejectsOrdinaryReconcileRunScript(t *testing.T) {
	...
	if _, err := testtaskplan.Build(input); !errors.Is(err, errs.New(errs.KindValidationFailed, "")) {
		t.Fatalf("BuildPlan(ordinary reconcile RunScript) error = %v, want validation.failed", err)
	}
}

func validPlanBuildInput() testtaskplan.BuildInput {
	...
	return testtaskplan.BuildInput{
