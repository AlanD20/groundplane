...
func TestOrdinaryRecreateCompilerKeepsPlanLocalPriorArtifactIdentity(t *testing.T) {
	...
	if capturedWorkload == nil || priorWorkload == nil ||
		...
		labelPairMap(capturedWorkload.ExpectedLabels)[testcomposerender.ComposeLabelReleaseID] != member.Intent.PriorServingReleaseID ||
		labelPairMap(priorWorkload.ExpectedLabels)[testcomposerender.ComposeLabelReleaseID] != member.Intent.PriorServingReleaseID {
