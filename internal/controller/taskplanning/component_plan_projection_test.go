...
func TestProjectPinnedEnvironmentComponentsRehydratesExactGraph(t *testing.T) {
	...
	if !exists || generated.Networks["frontend"].Ipv4Address != "10.70.0.2" ||
		len(
			result.PlainFiles,
		) != 1 || result.PlainFiles[0].ComponentID != projection.Components[0].Desired.ID || string(result.PlainFiles[0].Content) != projection.DesiredRoutes[0].Desired.ID+
...
x-gp-components:
  http-router:
	...
	projection.NormalizedCompose, err = testcomposerender.MarshalNormalizedEnvironmentProject(parsed.Project)
	if err != nil {
		t.Fatal(err)
	}
	reader := &blueprintPlanReader{
		...
		revision: testblueprints.EnvironmentBlueprintRevision{
			...
			Files:     []testblueprints.EnvironmentBlueprintFile{{Path: "blueprint.yaml", Content: content}},
	...
	actual, err := resolver.ResolveComponentFile(
		context.Background(),
		identity.EnvironmentID,
		testtaskmaterialization.ComponentFileValueReference{
...
func componentPlanProjectionInput(
	t *testing.T,
) (*composetypes.Project, pinnedEnvironmentIdentity, testenvironmentprojection.EnvironmentComposeProjection, []core.RouteSpec, []testcomponentrender.EnvironmentComponentRegistration) {
	t.Helper()
	...
	projection := testenvironmentprojection.EnvironmentComposeProjection{
		...
		DesiredRoutes: []testenvironmentprojection.EnvironmentRouteProjection{
			{EnvironmentID: environmentID, Desired: core.Route{
	...
	catalog := []testcomponentrender.EnvironmentComponentRegistration{
...
func appendMigrationIdentity(projection *testenvironmentprojection.EnvironmentComposeProjection) {
	at := time.Date(2026, 8, 23, 0, 0, 0, 0, time.UTC)
