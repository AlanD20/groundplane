...
func TestProjectCandidateScriptIncludesRetainedAttachNetwork(t *testing.T) {
	...
	sources := testscriptsourcequeries.ScriptExecutionSources{Revision: 20,
		DesiredProjection: testkeyvalue.Versioned[testenvironmentprojection.EnvironmentComposeProjection]{
			Record: testenvironmentprojection.EnvironmentComposeProjection{
				DesiredZones: []testenvironmentprojection.EnvironmentZoneProjection{
					{Desired: core.Zone{ID: owned, Name: "backend"}},
				},
			},
		},
		AttachSources: testscriptsourcequeries.ScriptAttachSources{
			Networks: []testkeyvalue.Versioned[testzones.Record]{
				{
					Record:       testzones.Record{Desired: core.Zone{ID: backing, Name: "database"}},
					Revision:     7,
					ReadRevision: 20,
				},
			},
...
func TestStripControllerServiceExtensionsConsumesRecognizedMetadataOnly(t *testing.T) {
	...
	normalize := func(extensions composetypes.Extensions) composetypes.ServiceConfig {
		t.Helper()
		encoded, err := composerender.MarshalNormalizedEnvironmentProject(&composetypes.Project{
			Services: composetypes.Services{"app": {Image: "example/app:1", Extensions: extensions}},
		})
		if err != nil {
			t.Fatal(err)
		}
		project, err := composerender.LoadNormalizedEnvironmentProject(
			t.Context(),
			testenvironmentprojection.EnvironmentComposeProjection{
				EnvironmentID: "env_01ARZ3NDEKTSV4RRFFQ69G5FAV", NormalizedCompose: encoded,
			},
		)
		if err != nil {
			t.Fatal(err)
		}
		return project.Services["app"]
	}
	service := normalize(composetypes.Extensions{
		"x-gp-resource": map[string]any{"id": "svc_exact"},
	...
	withUnknown := normalize(composetypes.Extensions{
		"x-gp-release": map[string]any{"default_strategy": "recreate"},
...
func TestProjectScriptNetworksOmitsServiceEndpointIdentity(t *testing.T) {
	...
	sources := testscriptsourcequeries.ScriptExecutionSources{
		...
		Networks: []testkeyvalue.Versioned[testzones.Record]{{
			Record:   testzones.Record{Desired: core.Zone{ID: networkID, Name: "backend"}},
			Revision: 7,
		}},
	}
...
func TestProjectScriptNetworksUsesCanonicalPhysicalName(t *testing.T) {
	...
	sources := testscriptsourcequeries.ScriptExecutionSources{
		...
		Networks: []testkeyvalue.Versioned[testzones.Record]{{
			Record: testzones.Record{Desired: core.Zone{ID: networkID, Name: "backend"}}, Revision: 7,
		}},
	}
	service := composetypes.ServiceConfig{Networks: map[string]*composetypes.ServiceNetworkConfig{
		"backend": {},
...
func TestScriptRunnerReleaseImageUsesSealedLocalIdentity(t *testing.T) {
	...
	sources := testscriptsourcequeries.ScriptExecutionSources{
		...
		RenderInput: testkeyvalue.Versioned[testreleaserender.ReleaseRenderInput]{
			Record: testreleaserender.ReleaseRenderInput{
				CandidateWorkload: seal, Projection: testenvironmentprojection.EnvironmentComposeProjection{
