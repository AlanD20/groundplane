...
func TestPrepareBlueprintReleaseTaskGlobalHookPhases(t *testing.T) {
	...
	project, err := testcomposerender.LoadNormalizedEnvironmentProject(t.Context(), reader.projection)
	if err != nil {
		t.Fatal(err)
	}
	...
	for index, service := range reader.projection.DesiredServices {
		...
		member := testreleaserender.ReleaseTaskRenderMember{
			...
			Render: testreleaserender.ReleaseRenderInput{
				ReleaseID: releaseID, PlanID: task.PlanID, ArtifactID: task.Params[testtaskcontract.EnvironmentBlueprintArtifactParam],
		...
		member.Render.Hooks = []testreleaserender.ReleaseHookRenderInput{
		...
		stepID := func(offset int64) string {
			return ids.NewAt(ids.KindStep, task.CreatedAt, int64(500+index*10)+offset)
		}
		...
		for hookIndex, hookStep := range []string{stepID(5), stepID(4)} {
			task.Params[testreleaserender.ReleaseHookStepMemberParam(hookStep)] = strconv.Itoa(index + 1)
			task.Params[testreleaserender.ReleaseHookStepExecutionParam(hookStep)] = member.Render.Hooks[hookIndex].ScriptExecutionID
		}
	}
...
func blueprintPhaseHookForTest(
	...
	sequence int64,
	explicit bool,
) testreleaserender.ReleaseHookRenderInput {
	t.Helper()
	...
	return testreleaserender.ReleaseHookRenderInput{
