...
func TestZoneRemovalPlanRebuildsAffectedServicesBeforeManagedNetworkRemoval(t *testing.T) {
	...
	task := etcd.TaskRecord{
		ID: taskID, OperationID: operationID, Executor: testtaskjournal.TaskExecutorAgent,
		PlanID: planID, RenderGeneration: 2, Type: testtaskjournal.TaskRemove, Target: zoneID,
		Params: map[string]string{
			testtaskjournal.TaskZoneEnvironmentParam:       environmentID,
			testtaskjournal.TaskZoneRemovalOperationParam:  operationID,
		...
		Steps: []testtaskjournal.TaskStepRecord{
			...
			}, {Kind: testtaskjournal.TaskStepOperation, ID: ids.NewAt(ids.KindStep, now, 10)},
			{Kind: testtaskjournal.TaskStepOperation, ID: ids.NewAt(ids.KindStep, now, 11)},
		},
		TimeoutSeconds: 120,
	}
	intent := testenvironmentchanges.ZoneRemovalIntent{
		...
		Status: testtaskjournal.TaskStatusPending, AffectedServiceIDs: []string{firstServiceID, secondServiceID},
