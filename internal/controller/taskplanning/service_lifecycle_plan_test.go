...
func TestServiceLifecyclePlanSelectsAppliedSealedAddressableRuntime(t *testing.T) {
	...
	source, err := testcomposerender.RenderCompose(testcomposerender.ComposeRenderInput{
		...
		Releases: map[string]testcomposerender.ComposeReleaseIdentity{serviceID: {
	...
	releaseRender := testreleaserender.ReleaseRenderInput{
	...
	task := etcd.TaskRecord{
		...
		Executor: testtaskjournal.TaskExecutorAgent, PlanID: input.PlanID, Type: testtaskjournal.TaskStart,
		Target: serviceID, TimeoutSeconds: 120, Status: testtaskjournal.TaskStatusPending,
		Params: map[string]string{
			testtaskjournal.TaskServiceEnvironmentParam: input.EnvironmentID,
			testtaskjournal.TaskComposeArtifactParam:    input.ArtifactID,
		},
...
func TestServiceLifecycleProcedureUsesTargetedComposeOperations(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{taskType: testtaskjournal.TaskStart, operation: agentpb.PlanOperation_PLAN_OPERATION_START, assert: func(t *testing.T, step *agentpb.ExecutionStep) {
		...
		{taskType: testtaskjournal.TaskStop, operation: agentpb.PlanOperation_PLAN_OPERATION_STOP, assert: func(t *testing.T, step *agentpb.ExecutionStep) {
		...
		{taskType: testtaskjournal.TaskDestroy, operation: agentpb.PlanOperation_PLAN_OPERATION_DESTROY, assert: func(t *testing.T, step *agentpb.ExecutionStep) {
		...
		task := etcd.TaskRecord{
			...
			Steps: []testtaskjournal.TaskStepRecord{
				{Kind: testtaskjournal.TaskStepOperation, ID: ids.New(ids.KindStep)},
			},
		}
...
func TestServiceLifecyclePlanCompilesPinnedStartDependenciesAcrossRestart(t *testing.T) {
	...
	task := etcd.TaskRecord{
		...
		Executor: testtaskjournal.TaskExecutorAgent, PlanID: input.PlanID, Type: testtaskjournal.TaskStart,
		Target: apiID, TimeoutSeconds: 120, Status: testtaskjournal.TaskStatusPending,
	}
