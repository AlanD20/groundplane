...
func componentFixtureDigest(t *testing.T, value string) []byte {
	t.Helper()
...
func TestPrepareBlueprintReleaseTaskFirstCandidateAuthorityIsPredecessorIndependent(t *testing.T) {
	...
	task.Params[testreleaserender.TaskReleasePublicationParam] = "publication"
	member := testreleaserender.ReleaseTaskRenderMember{
		...
		Render: testreleaserender.ReleaseRenderInput{
			ReleaseID: candidateReleaseID, PlanID: task.PlanID, ArtifactID: task.Params[testtaskcontract.EnvironmentBlueprintArtifactParam],
...
func TestPrepareBlueprintReleaseTaskOwnsForwardStepPolicies(t *testing.T) {
	...
	artifactID := task.Params[testtaskcontract.EnvironmentBlueprintArtifactParam]
	...
	projectionArtifact.Services = append(projectionArtifact.Services, &agentpb.ComposeService{
		...
		ImageIndexDigest: componentFixtureDigest(t,
			selected.IndexDigest,
		), ImageChildDigest: componentFixtureDigest(t, selected.Platform.ChildDigest),
		ImageConfigDigest: componentFixtureDigest(t, selected.Platform.ConfigDigest), ImageOs: selected.Platform.OS,
		ImageArchitecture: selected.Platform.Architecture, ImageVariant: selected.Platform.Variant,
	})
	project, err := testcomposerender.LoadNormalizedEnvironmentProject(context.Background(), reader.projection)
	if err != nil {
		t.Fatal(err)
	}
	...
	identities, err := testcomposerender.ComposeIdentitySnapshotFromProjection(reader.projection)
	if err != nil {
		t.Fatal(err)
	}
	...
	projectionArtifact, err = testcomposerender.RenderCompose(testcomposerender.ComposeRenderInput{
		...
		ExternalNetworks: []testcomposeidentity.Resource{{ID: "net_01ARZ3NDEKTSV4RRFFQ69G5FB0", Name: attachName}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	reader.projection.Components = []testcomponents.Record{{
	...
	prefix, err := testtaskmaterialization.BuildTaskMaterializationStep(
	...
	input := BlueprintReleasePlanInput{
		Members: []testreleaserender.ReleaseTaskRenderMember{member}, PrefixSteps: []*agentpb.ExecutionStep{prefix},
...
func TestBindBlueprintCandidateServiceImagesRejectsDuplicateCandidateWorkload(t *testing.T) {
	...
	labels := []*agentpb.LabelPair{{Key: testcomposerender.ComposeLabelReleaseID, Value: releaseID}}
