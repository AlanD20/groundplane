...
type entryRemovalPlanReader struct {
	*blueprintPlanReader
	intent testenvironmentchanges.EntryRemovalIntent
}

func (reader *entryRemovalPlanReader) GetEntryRemovalIntent(
	context.Context, string,

) (testkeyvalue.Versioned[testenvironmentchanges.EntryRemovalIntent], bool, error) {
	return testkeyvalue.Versioned[testenvironmentchanges.EntryRemovalIntent]{Record: reader.intent}, true, nil
}
...
func (fake entryRemovalMaterializationResolverFake) ResolveTaskMaterializationSource(
	...
	if source.Kind == testtaskmaterialization.SourceRemoval {
		return nil, nil
	}
...
func TestEntryRemovalMaterializationTemplatesCloseEnvironmentScopes(t *testing.T) {
	...
	tests := []struct {
		...
		wantKind   testtaskmaterialization.OutputKind
		wantValues int
		wantName   string
	}{
		{
			...
			wantKind: testtaskmaterialization.OutputGeneratedEnvironment,
		},
		{
			name:     "service file removed when empty",
			removed:  core.EnvEntry{Kind: core.EntryKindEnv, Key: "TOKEN", Exposure: []string{"api"}},
			wantKind: testtaskmaterialization.OutputRemoveGeneratedEnv,
		},
		{
			...
			wantKind: testtaskmaterialization.OutputGeneratedEnvironment, wantValues: 1, wantName: "TOKEN",
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			values := []testtaskmaterialization.GeneratedEnvironmentEntryReference(nil)
...
func TestEntryRemovalMaterializationTemplateUsesDesiredServiceIdentity(t *testing.T) {
	...
	if template.ServiceID != intent.CandidateProjection.DesiredServices[0].Desired.ID ||
		template.ServiceName != "api" || template.Destination != testenvironmentfile.ServiceEnvFileName(intent.EnvironmentID, "api") {
...
func TestEntryRemovalMaterializationTemplateResolvesGeneratedComponentService(t *testing.T) {
	...
	if template.ServiceID != generatedServiceID || template.ServiceName != "caddy" ||
		template.Destination != testenvironmentfile.ServiceEnvFileName(intent.EnvironmentID, "caddy") {
...
func TestEntryRemovalMaterializationConvertsClosedKinds(t *testing.T) {
	t.Parallel()
	outputs := []struct {
		input testtaskmaterialization.OutputKind
		want  entrycapability.RemovalOutputKind
	}{
		{
			input: testtaskmaterialization.OutputGeneratedEnvironment,
			want:  entrycapability.RemovalOutputGeneratedEnvironment,
		},
		{input: testtaskmaterialization.OutputPlainFile, want: entrycapability.RemovalOutputPlainFile},
		{input: testtaskmaterialization.OutputSecretFile, want: entrycapability.RemovalOutputSecretFile},
		{
			input: testtaskmaterialization.OutputRemoveGeneratedEnv,
			want:  entrycapability.RemovalOutputRemoveGeneratedEnv,
		},
		{input: testtaskmaterialization.OutputRemovePlainFile, want: entrycapability.RemovalOutputRemovePlainFile},
		{input: testtaskmaterialization.OutputRemoveSecretFile, want: entrycapability.RemovalOutputRemoveSecretFile},
	}
	for _, test := range outputs {
		converted, err := entryRemovalMaterialization(testtaskmaterialization.Record{
	...
	for _, storage := range []struct {
		input testtaskmaterialization.EntryValueStorage
		want  entrycapability.RemovalValueStorage
	}{
		{input: testtaskmaterialization.EntryValueStoragePlain, want: entrycapability.RemovalValueStoragePlain},
		{input: testtaskmaterialization.EntryValueStorageSecret, want: entrycapability.RemovalValueStorageSecret},
	} {
	...
	if _, err := entryRemovalMaterialization(testtaskmaterialization.Record{
		OutputKind: "future",
		Source:     testtaskmaterialization.Source{Kind: testtaskmaterialization.SourceRemoval},
	}); err == nil {
...
func generatedEntryRemovalMaterialization(
	storage testtaskmaterialization.EntryValueStorage,
) testtaskmaterialization.Record {
	return testtaskmaterialization.Record{
		...
		Source: testtaskmaterialization.Source{
			...
			GeneratedEnvironment: &testtaskmaterialization.GeneratedEnvironmentValueReference{
				Values: []testtaskmaterialization.GeneratedEnvironmentEntryReference{{
					Value: testtaskmaterialization.EntryValueReference{Storage: storage},
				}},
			},
		},
	}
...
func entryRemovalPlanTestState(
	...
	removed core.EnvEntry,
	remaining []core.EnvEntry,
) (*entryRemovalPlanReader, testenvironmentchanges.EntryRemovalIntent, etcd.TaskRecord) {
	t.Helper()
	...
	var err error
	projection.NormalizedCompose, err = testcomposerender.MarshalNormalizedEnvironmentProject(project)
	if err != nil {
		t.Fatal(err)
	}
	projection.DesiredServices = slices.DeleteFunc(projection.DesiredServices,
		func(service testservices.EnvironmentServiceProjection) bool {
	...
	removedRecord, err := testentries.NewRecord(
	...
	for index := range remaining {
		...
		record, recordErr := testentries.NewRecord(
			projection.EnvironmentID,
			remaining[index],
	...
	task := etcd.TaskRecord{
		ID: taskID, OperationID: ids.NewAt(ids.KindOperation, at, 31), Executor: testtaskjournal.TaskExecutorAgent,
		PlanID: ids.NewAt(ids.KindPlan, at, 32), Type: testtaskjournal.TaskRemove, Target: removed.ID,
		TimeoutSeconds: 120, Status: testtaskjournal.TaskStatusPending, CreatedAt: at,
		Owner: testtaskjournal.TaskOwner{
			WorkspaceType: testtaskjournal.TaskWorkspaceTenant, TenantID: baseReader.tenant.ID,
...
func sortEntryRecords(records []testentries.Record) {
