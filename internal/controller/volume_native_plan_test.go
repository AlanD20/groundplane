...
func TestVolumeResourcePlanPreservesRenderedNativeRelease(t *testing.T) {
	...
	current := baseline
	for _, action := range []VolumeArtifactAction{VolumeArtifactAdd, VolumeArtifactEdit} {
		candidate, err := MutateEnvironmentVolumeArtifact(current, VolumeArtifactMutation{Action: action,
		...
		steps = append(steps, &agentpb.ExecutionStep{StepId: "step_01ARZ3NDEKTSV4RRFFQ69G5FC7", TimeoutSeconds: 30,
			Payload: &agentpb.ExecutionStep_ManagedVolumeEnsure{ManagedVolumeEnsure: &agentpb.ManagedVolumeEnsure{
				ArtifactId: artifactID, VolumeId: volumeID, RequireExisting: action == VolumeArtifactEdit,
			}}})
		if _, err := BuildPlan(PlanBuildInput{VolumeRoot: "/var/lib/groundplane/vol", PlanID: planID,
		...
		if action == VolumeArtifactEdit && !bytes.Equal(current.CanonicalYaml, candidate.CanonicalYaml) {
