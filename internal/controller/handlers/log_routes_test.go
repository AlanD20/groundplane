...
import (
	...
	testservicelogs "github.com/AlanD20/groundplane/internal/controller/servicelogs"
...
func TestLogRouteRejectsAcceptResumeAndBodyViolationsBeforeHeaders(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			server.logs = &testservicelogs.Service{}
...
func TestPublicLogEventAcceptsExactShapeAndRejectsInvalidAgentData(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "invalid utf8",
			mutate: func(event *agentpb.LogEvent) *agentpb.LogEvent {
				event.Line = string([]byte{0xff})
				return event
			},
		},
