...
type fakeConnectorReader struct {
	record          testkeyvalue.Versioned[testconnectors.Record]
...
func (fake *fakeConnectorReader) ListConnectors(
	...
	if environmentID != fake.wantEnvironment || request != fake.wantPage {
		return testkeyvalue.Page[testconnectors.Record]{}, io.ErrUnexpectedEOF
	}
...
func TestConnectorRoutesExposeRedactedListAndDetail(t *testing.T) {
	...
	reader := &fakeConnectorReader{
		record: testkeyvalue.Versioned[testconnectors.Record]{Record: record},
		page: testkeyvalue.Page[testconnectors.Record]{
			Items: []testkeyvalue.Versioned[testconnectors.Record]{{Record: record}}, NextCursor: "next",
		},
		wantEnvironment: environmentID,
		wantPage:        testkeyvalue.PageRequest{Limit: 3, Cursor: "opaque"},
	}
