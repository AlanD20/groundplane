...
type routeServiceObserver struct {
	...
	calls  [][]testkeyvalue.Versioned[testservices.ServiceRecord]
}

func (observer *routeServiceObserver) ObserveServices(
	_ context.Context, records []testkeyvalue.Versioned[testservices.ServiceRecord],
...
func TestServiceObservationListAndShowHTTP(t *testing.T) {
	...
	record := testkeyvalue.Versioned[testservices.ServiceRecord]{Record: testservices.ServiceRecord{
	...
	reader := &fakeServiceReader{
		...
		page: testkeyvalue.Page[testservices.ServiceRecord]{
			Items: []testkeyvalue.Versioned[testservices.ServiceRecord]{record}, Revision: 40,
		},
	}
