...
func (mutations *scriptOrderRouteMutations) EditScript(
	...
	mutations.calls++
	if input.Order == nil {
		return testidempotencyowner.IdempotencyResponse{}, errs.New(errs.KindValidationFailed, "missing order")
	}
	return scriptOrderRouteResponse(*input.Order, http.StatusOK)
}

func (*scriptOrderRouteMutations) RemoveScript(
	context.Context,
	string,
	string,
) (testidempotencyowner.IdempotencyResponse, error) {
	return testidempotencyowner.IdempotencyResponse{}, errs.New(errs.KindNotImplemented, "not part of order authoring")
}

func (*scriptOrderRouteMutations) RunScript(
	context.Context,
	string,
	string,
) (testidempotencyowner.IdempotencyResponse, error) {
...
func scriptOrderRouteResponse(order uint16, status int) (testidempotencyowner.IdempotencyResponse, error) {
	body, err := json.Marshal(apiTypes.Script{Order: order, Execution: apiTypes.ScriptExecution{Mode: "inherited"}})
	return testidempotencyowner.IdempotencyResponse{Status: status, ContentKind: "application/json", Body: body}, err
}
