...
func (fake *fakeEntryReader) ListEntries(
	...
	if environmentID != fake.wantEnvironment || request != fake.wantPage {
		return testkeyvalue.Page[testentries.Record]{}, io.ErrUnexpectedEOF
	}
...
func TestEntryRoutesFlattenMetadataAndRevealExplicitly(t *testing.T) {
	...
	pageRequest := testkeyvalue.PageRequest{Limit: 5, Cursor: "opaque"}
	reader := &fakeEntryReader{
		record: testkeyvalue.Versioned[testentries.Record]{Record: record},
		page: testkeyvalue.Page[testentries.Record]{
...
func TestEntryCreateRoutePreservesProtectedMutationResponse(t *testing.T) {
	...
	want := testidempotencyowner.IdempotencyResponse{
