...
func (fake *fakeZoneDeletionRepository) GetEnvironmentZoneRemovalAuthorities(
	context.Context, string,
) (testenvironmentchanges.EnvironmentZoneRemovalAuthorities, bool, error) {
	return fake.authorities, true, nil
}

func (fake *fakeZoneDeletionRepository) ClaimEnvironmentBlueprintStage(
	_ context.Context, request testblueprints.EnvironmentBlueprintStageClaimRequest,
...
func (fake *fakeZoneDeletionRepository) StageEnvironmentBlueprintRevision(
	context.Context, testblueprints.EnvironmentBlueprintStageRequest,
...
func (fake *fakeZoneDeletionRepository) BeginZoneDeletionWithTask(
	...
	_ testkeyvalue.Versioned[testzones.Record],
	_ testenvironmentchanges.EnvironmentZoneRemovalAuthorities,
	tombstone testdeletions.DeletionTombstoneRecord,
	intent testenvironmentchanges.ZoneRemovalIntent,
	task etcd.TaskRecord,
	marker testidempotency.IdempotencyMarker,
...
func (*fakeZoneDeletionIdempotency) Prepare(
	context.Context, testidempotency.IdempotencyLocator, string, string,
...
func (*fakeZoneDeletionIdempotency) MatchesStaged(
	context.Context, zoneDeletionEvidence, testidempotency.ProtectedIntentRecord,
) (bool, error) {
	return true, nil
}
	...
	return testkeyvalue.Versioned[testenvironmentprojection.EnvironmentComposeProjection]{
		Record: testenvironmentprojection.EnvironmentComposeProjection{
			...
			DesiredZones: []testenvironmentprojection.EnvironmentZoneProjection{
				{EnvironmentID: zone.EnvironmentID, Desired: zone.Desired},
			},
		},
		Revision:     20,
		ReadRevision: 20,
...
func TestZoneDeletionPublishesExactTaskAndReplayTarget(t *testing.T) {
	...
	repository := &fakeZoneDeletionRepository{
		zone: testkeyvalue.Versioned[testzones.Record]{Record: testzones.Record{
		...
		project: testkeyvalue.Versioned[testhierarchy.ProjectRecord]{Record: testhierarchy.ProjectRecord{
			ID: projectID, TenantID: ids.NewAt(ids.KindTenant, now, 4), Kind: testhierarchy.ProjectKindTenant,
			Slug: "web", Name: "Web",
	...
	if json.Unmarshal(response.Body, &accepted) != nil || accepted.TaskID != repository.task.ID ||
		...
		repository.task.Params[testtaskjournal.TaskZoneEnvironmentParam] != environmentID ||
		repository.marker.ReplayTarget == nil ||
		repository.marker.ReplayTarget.Kind != testidempotency.IdempotencyReplayTargetZone ||
...
func TestBackingZoneDeletionPublishesCascadeParent(t *testing.T) {
	...
	repository := &fakeZoneDeletionRepository{
		...
		project: testkeyvalue.Versioned[testhierarchy.ProjectRecord]{Record: testhierarchy.ProjectRecord{
			ID: projectID, Kind: testhierarchy.ProjectKindBacking, Slug: "postgres", Name: "Postgres",
	...
	if repository.task.Executor != testtaskjournal.TaskExecutorController ||
		repository.task.Params[testtaskjournal.TaskResourceKindParam] != testtaskjournal.TaskResourceBackingZone ||
		repository.task.Params[testtaskjournal.TaskZoneEnvironmentParam] != environmentID ||
		repository.task.Params[testtaskjournal.TaskZoneImpactTokenParam] != impactToken ||
