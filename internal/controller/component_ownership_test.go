...
func testSelectedComponentImage(repository string) SelectedComponentImage {
...
func TestComposeIdentitySnapshotPreservesSelectedComponentImage(t *testing.T) {
	...
	selected := SelectedComponentImage{
	...
	for _, mutation := range []string{"none", "missing config", "mismatched reference", "invalid platform"} {
		t.Run(mutation, func(t *testing.T) {
			...
			projection := etcd.EnvironmentComposeProjection{ComposeArtifact: encoded,
