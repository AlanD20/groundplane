	...
	for _, action := range []string{volumeMutationActionAdd, volumeMutationActionEdit, volumeMutationActionRemove} {
		for _, slot := range []domain.WorkloadTarget{domain.WorkloadBlue, domain.WorkloadGreen} {
			t.Run(action+"/"+string(slot), func(t *testing.T) {
				...
				baseline.DesiredServices = []testservices.EnvironmentServiceProjection{{EnvironmentID: environment.ID,
					Desired: core.Service{ID: serviceID, Name: "api", Image: "api:1"}}}
				for _, volume := range baseline.Volumes {
					baseline.VolumeMounts = append(
						baseline.VolumeMounts,
						testenvironmentprojection.EnvironmentServiceVolumeMount{
							ServiceID: serviceID, VolumeID: volume.ID, Target: "/" + volume.Key,
						},
					)
				}
				if _, err := testenvironmentprojection.EncodeEnvironmentComposeProjectionStorage(baseline); err != nil {
					t.Fatalf("baseline: %v", err)
				}
				volume := baseline.Volumes[1]
				if action == volumeMutationActionAdd {
					volume = testenvironmentprojection.EnvironmentVolumeIdentity{Key: "cache", Slug: "cache"}
				}
				...
				if _, err := testenvironmentprojection.EncodeEnvironmentComposeProjectionStorage(candidate); err != nil {
