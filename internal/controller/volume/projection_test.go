...
func TestVolumeMutationPreservesBackupPolicyDecisions(t *testing.T) {
	...
	for _, test := range []struct{ action, volumeID, key, slug string }{
		...
		t.Run(test.action, func(t *testing.T) {
			...
			if _, err := testblueprints.EnvironmentBlueprintDependencyDigest(candidate); err != nil {
...
func volumePolicyProjectionFixture(
	t *testing.T,
) (string, string, testhierarchy.EnvironmentRecord, testenvironmentprojection.EnvironmentComposeProjection) {
	t.Helper()
	at := time.Date(2026, 9, 9, 0, 0, 0, 0, time.UTC)
	tenantID, projectID := ids.NewAt(ids.KindTenant, at, 1), ids.NewAt(ids.KindProject, at, 2)
	environment := testhierarchy.EnvironmentRecord{ID: ids.NewAt(ids.KindEnvironment, at, 3), CreatedAt: at}
	environment.VolumeDir = "/var/lib/groundplane/vol/" + tenantID + "/" + projectID + "/" + environment.ID
	baseline := testenvironmentprojection.EnvironmentComposeProjection{}
	...
	if _, err := testblueprints.EnvironmentBlueprintDependencyDigest(baseline); err != nil {
