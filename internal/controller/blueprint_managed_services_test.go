...
func TestBlueprintManagedStartupReplaysBeforeFullProjectHealth(t *testing.T) {
	...
	for index, name := range []string{"router", "tunnel"} {
		...
		input.Identities.Services = append(input.Identities.Services, ComposeResourceIdentity{
			...
			ComponentImage: &SelectedComponentImage{Repository: image.Repository,
	...
	for _, step := range steps {
		task.Steps = append(task.Steps, etcd.TaskStepRecord{ID: step.StepId, Kind: etcd.TaskStepOperation})
	}
