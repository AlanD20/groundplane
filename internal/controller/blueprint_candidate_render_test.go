	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			input := etcd.ReleaseRenderInput{
				PlanID: task.PlanID, ArtifactID: task.Params[EnvironmentBlueprintArtifactParam],
			...
			_, err = renderBlueprintCandidateArtifact(t.Context(), task, input,
				map[string]domain.WorkloadSeal{"api": seal}, map[string]ComposeReleaseIdentity{
...
func freezeBlueprintNativeRuntimeFixture(
	...
	artifact, err := RenderCompose(ComposeRenderInput{
		Project: project, ArtifactID: task.Params[EnvironmentBlueprintArtifactParam],
