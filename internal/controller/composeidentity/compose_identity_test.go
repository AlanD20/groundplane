...
func TestReconcileOwnedComposeIdentitiesPreservesCreatesAndRemoves(t *testing.T) {
	...
	previous := Snapshot{
		Services: []Resource{{ID: serviceAPI, Name: "api"}, {ID: serviceRemoved, Name: "old"}},
		Networks: []Resource{
			{ID: networkFrontend, Name: "frontend"},
			{ID: networkRemoved, Name: "old"},
		},
		Volumes: []Resource{{ID: volumeData, Name: "data"}, {ID: volumeRemoved, Name: "old"}},
	}
	...
	changes, err := ReconcileOwned(project, previous, func(kind ids.Kind) string {
	...
	want := Changes{
		Current: Snapshot{
			Services: []Resource{{ID: serviceAPI, Name: "api"}, {ID: serviceWorker, Name: "worker"}},
			Networks: []Resource{
				{ID: networkBackend, Name: "backend"},
				{ID: networkFrontend, Name: "frontend"},
			},
			Volumes: []Resource{{ID: volumeCache, Name: "cache"}, {ID: volumeData, Name: "data"}},
		},
...
func TestReconcileOwnedComposeIdentitiesDoesNotInferRename(t *testing.T) {
	...
	previous := Snapshot{Services: []Resource{{ID: oldID, Name: "api"}}}

	changes, err := ReconcileOwned(project, previous, func(ids.Kind) string { return newID })
	if err != nil {
		t.Fatalf("ReconcileOwnedComposeIdentities() error = %v", err)
	}
	if got := changes.Current.Services; !reflect.DeepEqual(got, []Resource{{ID: newID, Name: "web"}}) {
...
func TestReconcileOwnedComposeIdentitiesRejectsCorruptSnapshot(t *testing.T) {
	...
	cases := map[string]Snapshot{
		"duplicate name": {
			Services: []Resource{{ID: serviceA, Name: "api"}, {ID: serviceB, Name: "api"}},
		},
		"duplicate id": {
			Services: []Resource{{ID: serviceA, Name: "api"}, {ID: serviceA, Name: "worker"}},
		},
		"wrong id kind": {
			Services: []Resource{{ID: composeIdentityTestID(ids.KindNetwork, 14), Name: "api"}},
		},
	}
	for name, previous := range cases {
		t.Run(name, func(t *testing.T) {
			_, err := ReconcileOwned(&composetypes.Project{}, previous, ids.New)
...
func TestReconcileOwnedComposeIdentitiesRejectsDuplicateActiveAndDisabledService(t *testing.T) {
	...
	_, err := ReconcileOwned(project, Snapshot{}, ids.New)
...
func TestReconcileOwnedComposeIdentitiesExcludesExternalNetworks(t *testing.T) {
	...
	changes, err := ReconcileOwned(project, Snapshot{}, func(ids.Kind) string {
...
func TestReconcileOwnedComposeIdentitiesRejectsInvalidGeneratedID(t *testing.T) {
	...
	project := &composetypes.Project{Services: composetypes.Services{"web": composetypes.ServiceConfig{}}}
	previous := Snapshot{Services: []Resource{{ID: oldID, Name: "old"}}}
	...
	for name, generated := range cases {
		t.Run(name, func(t *testing.T) {
			_, err := ReconcileOwned(project, previous, func(ids.Kind) string { return generated })
			if !errors.Is(err, errs.New(errs.KindInternal, "")) {
				t.Fatalf("ReconcileOwnedComposeIdentities() error = %v, want %q", err, errs.CodeInternal)
			}
		})
