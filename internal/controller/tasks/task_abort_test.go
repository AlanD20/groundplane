...
func (repository *fakeTaskAbortRepository) AbortPendingTask(
	...
	repository.task.Status = testtaskjournal.TaskStatusAborted
	return testkeyvalue.Versioned[etcd.TaskRecord]{Record: repository.task}, nil
}
...
func TestTaskAbortServiceRejectsInternalBackupPruneBeforeMutation(t *testing.T) {
	...
	repository := &fakeTaskAbortRepository{task: etcd.TaskRecord{
		ID: taskID, Type: testtaskjournal.TaskBackupPrune, Status: testtaskjournal.TaskStatusPending,
	}}
	service, err := NewAbortService(
		repository,
		&fakeTaskAbortAgentChannel{terminal: make(chan error, 1)},
		&fakeTaskAbortControllerRunner{repository: repository},
	)
	if err != nil {
		t.Fatalf("NewAbortService() error = %v", err)
	}
	...
	if repository.abortPendingCalls != 0 || repository.task.Status != testtaskjournal.TaskStatusPending {
...
func (channel *fakeTaskAbortAgentChannel) AbortTask(
	...
	channel.repository.task.Status = testtaskjournal.TaskStatusAborted
...
func (runner *fakeTaskAbortControllerRunner) AbortTask(context.Context, string) error {
	runner.repository.task.Status = testtaskjournal.TaskStatusAborted
	return nil
}
...
func TestTaskAbortServiceTerminalizesPendingTaskAndReturnsItsIdentity(t *testing.T) {
	...
	repository := &fakeTaskAbortRepository{task: etcd.TaskRecord{ID: taskID, Status: testtaskjournal.TaskStatusPending}}
	service, err := NewAbortService(
		repository,
		&fakeTaskAbortAgentChannel{terminal: make(chan error, 1)},
		&fakeTaskAbortControllerRunner{repository: repository},
	...
	if repository.task.Status != testtaskjournal.TaskStatusAborted {
...
func TestTaskAbortServiceSubscribesBeforeFencedAgentDelivery(t *testing.T) {
	...
	record := testtaskassignments.TaskAssignmentRecord{
		...
		TaskID:       taskID, Executor: testtaskjournal.TaskExecutorAgent, AgentID: agentID, AgentGeneration: 4,
		ClaimedTaskRevision: 1, AssignedAt: now, Deadline: now.Add(time.Minute),
	}
	repository := &fakeTaskAbortRepository{
		task: etcd.TaskRecord{ID: taskID, Status: testtaskjournal.TaskStatusRunning},
		assignment: etcd.TaskAssignment{
			Assignment: testkeyvalue.Versioned[testtaskassignments.TaskAssignmentRecord]{Record: record},
			Task: testkeyvalue.Versioned[etcd.TaskRecord]{Record: etcd.TaskRecord{
				ID: taskID, Status: testtaskjournal.TaskStatusRunning,
			}},
		},
	}
