...
type tenantChangeIdempotency interface {
	PrepareEdit(context.Context, string, EditTenantInput) (tenantChangeEvidence, error)
...
func NewTenantChangeIdempotency(
	coordinator *requestidempotency.Coordinator,
	repository *etcd.IdempotencyRepository,
...
func (service *durableTenantChangeIdempotency) PrepareEdit(
	...
	if input.Description != nil {
		fields = append(fields, requestidempotency.Field{
			Name: "description", Value: requestidempotency.String(*input.Description),
		})
	}
	if input.Name != nil {
		fields = append(fields, requestidempotency.Field{Name: "name", Value: requestidempotency.String(*input.Name)})
	}
	return service.prepare(ctx, http.MethodPatch, tenantEditRoute, id, requestidempotency.Object(fields...))
}
...
func (service *durableTenantChangeIdempotency) PrepareRename(
	...
	return service.prepare(
		...
		tenantRenameRoute,
		id,
		requestidempotency.Object(requestidempotency.Field{Name: "slug", Value: requestidempotency.String(input.Slug)}),
	)
}
...
func (service *tenantChangeService) EditTenant(
	...
	return service.changeTenant(ctx, id, http.MethodPatch, tenantEditRoute, idempotencyKey,
		...
		func(current core.Tenant) (core.Tenant, error) { return PrepareTenantEdit(current, input) },
	)
}
...
func (service *tenantChangeService) RenameTenant(
	...
	return service.changeTenant(ctx, id, http.MethodPost, tenantRenameRoute, idempotencyKey,
		...
		func(current core.Tenant) (core.Tenant, error) { return PrepareTenantRename(current, input) },
	)
}
...
func (service *tenantChangeService) changeTenantOnce(
	...
	result, mutationErr := service.repository.MutateTenantIdempotent(ctx, current, hierarchyrecord.TenantRecord{
		ID: replacement.ID, Slug: replacement.Slug,
		Name: replacement.Name, Description: replacement.Description,
	}, marker)
	if mutationErr != nil {
