...
func TestSecretCreationSealsValueAndReturnsRedactedMetadata(t *testing.T) {
	t.Parallel()
	repository := &fakeSecretCreationRepository{project: testkeyvalue.Versioned[testhierarchy.ProjectRecord]{
		Record:   testhierarchy.ProjectRecord{ID: secretCreationTestProjectID, Kind: testhierarchy.ProjectKindTenant},
	...
	service, err := NewCreationService(repository, protector, idempotency)
	if err != nil {
...
func TestSecretCreationReplaysBeforeRepositoryAccess(t *testing.T) {
	...
	service, err := NewCreationService(repository, protector, &fakeSecretCreationIdempotency{
...
func secretCreationTestEvidence() secretCreationEvidence {
	...
	return secretCreationEvidence{durable: testidempotency.ProtectedIntentRecord{
