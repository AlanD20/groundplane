...
func (agent *scriptPreparationAgent) GetSingleton(context.Context) (etcd.Versioned[etcd.LocalAgentRecord], error) {
	agent.calls++
...
func scriptPreparationSources() etcd.ScriptExecutionSources {
...
func TestScriptRunnerPreparationBindsExactSource(t *testing.T) {
	for _, edit := range []func(*etcd.ScriptExecutionSources){
		func(s *etcd.ScriptExecutionSources) {
			s.Script.Record.Desired.Execution.Image = "example/other@sha256:" + strings.Repeat("d", 64)
		},
		func(s *etcd.ScriptExecutionSources) { s.Script.Record.Desired.Execution.User = "1:1" },
		func(s *etcd.ScriptExecutionSources) { s.Script.Record.Desired.ID = "scr_01ARZ3NDEKTSV4RRFFQ69G5FAW" },
		func(s *etcd.ScriptExecutionSources) { s.Script.Revision++ },
		func(s *etcd.ScriptExecutionSources) {
			s.Revision++
