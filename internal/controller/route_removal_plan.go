...
func (resolver *TaskPlanResolver) PrepareRouteRemovalTask(ctx context.Context, task etcd.TaskRecord, intent etcd.RouteRemovalIntent, procedure RouteRemovalTaskProcedureIDs) (etcd.RouteRemovalTaskPreparation, error) {
	if intent.CandidateProjection == nil {
		return etcd.RouteRemovalTaskPreparation{Intent: intent, Task: task}, nil
	}
	pin, err := resolver.pinRouteProvider(ctx, intent.EnvironmentID, intent.CurrentProjectionRevision, *intent.CandidateProjection, nil, intent.RouteID, intent.CandidateProjection.RenderGeneration)
	if err != nil {
	...
	intent.Provider = pin
	if ids.Validate(ids.KindConfig, procedure.ArtifactID) != nil || ids.Validate(ids.KindConfig, procedure.MaterializationID) != nil || ids.Validate(ids.KindStep, procedure.MaterializeStepID) != nil || ids.Validate(ids.KindStep, procedure.ComposeApplyStepID) != nil || ids.Validate(ids.KindStep, procedure.ActivateStepID) != nil || intent.CandidateProjection.RenderGeneration > math.MaxInt32 {
		return etcd.RouteRemovalTaskPreparation{}, errs.New(errs.KindValidationFailed, "Route removal procedure ids are invalid")
	}
	...
	length := len(content)
	clear(content)
	reference := etcd.TaskComponentFileValueReference{RevisionID: intent.CandidateProjection.RevisionID, ComponentID: pin.ComponentID, Path: pin.Destination, RouteTaskID: task.ID}
...
func (resolver *TaskPlanResolver) resolveRouteRemovalPlan(ctx context.Context, task etcd.TaskRecord) (*agentpb.ExecutionPlan, error) {
...
func (resolver *TaskPlanResolver) buildRouteRemovalPlan(ctx context.Context, task etcd.TaskRecord, intent etcd.RouteRemovalIntent) (*agentpb.ExecutionPlan, error) {
	if resolver == nil || resolver.blueprints == nil || task.Executor != etcd.TaskExecutorAgent || task.Type != etcd.TaskRemove || ids.Validate(ids.KindRoute, task.Target) != nil || task.ID != intent.TaskID || task.Target != intent.RouteID || !task.CreatedAt.Equal(intent.CreatedAt) || intent.Status != etcd.TaskStatusPending || intent.Provider == nil || intent.CandidateProjection == nil || intent.CurrentProjection == nil || task.TimeoutSeconds <= 0 || task.TimeoutSeconds > math.MaxUint32 || len(task.Params) != 4 || len(task.Steps) != 3 || len(task.Materializations) != 1 {
	...
	if task.Params[etcd.TaskRouteEnvironmentParam] != intent.EnvironmentID || task.Params[etcd.TaskMaterializationEnvironmentParam] != intent.EnvironmentID || revisionID != candidate.RevisionID || ids.Validate(ids.KindTask, revisionID) != nil || ids.Validate(ids.KindConfig, artifactID) != nil || uint64(task.RenderGeneration) != candidate.RenderGeneration || ids.Validate(ids.KindStep, task.Steps[0].ID) != nil || ids.Validate(ids.KindStep, task.Steps[1].ID) != nil || ids.Validate(ids.KindStep, task.Steps[2].ID) != nil {
	...
	if reference.StepID != task.Steps[0].ID || reference.EnvironmentID != intent.EnvironmentID || reference.Destination != pin.Destination || reference.ServiceID != "" || reference.ServiceName != "" || reference.OutputKind != etcd.TaskMaterializationOutputPlainFile || reference.UID != 0 || reference.GID != 0 || reference.Mode != uint32(entrymaterialization.ModeReadOnly) || reference.Source.Kind != etcd.TaskMaterializationSourceComponentFile || componentFile == nil || componentFile.RevisionID != revisionID || componentFile.ComponentID != pin.ComponentID || componentFile.Path != pin.Destination || componentFile.RouteTaskID != task.ID {
	...
	if environment.Record.ID != intent.EnvironmentID || environment.Record.ProvisioningState != etcd.EnvironmentProvisioningReady || project.Record.ID != environment.Record.ProjectID || project.Record.Kind != etcd.ProjectKindTenant || tenant.Record.ID != project.Record.TenantID {
	...
	action, err := BuildPinnedEnvironmentComponentAction(resolver.componentCatalog, pin.ComponentID, definitionPin, catalogPin, componentsdk.ActionID(pin.ActionID), reference.MaterializationID, digest, uint64(task.RenderGeneration))
	if err != nil {
		return nil, err
	}
	return BuildPlan(PlanBuildInput{VolumeRoot: resolver.volumeRoot, PlanID: task.PlanID, RenderGeneration: uint64(task.RenderGeneration), Operation: agentpb.PlanOperation_PLAN_OPERATION_REMOVE, TargetID: task.Target, Artifacts: []*agentpb.ComposeArtifact{artifact}, Steps: []*agentpb.ExecutionStep{materializationStep, {StepId: task.Steps[1].ID, TimeoutSeconds: uint32(task.TimeoutSeconds), PrerequisiteStepId: materializationStep.GetStepId(), Payload: &agentpb.ExecutionStep_ComposeApply{ComposeApply: &agentpb.ComposeApply{ArtifactId: artifactID, ServiceIds: []string{pin.ServiceID}, ForceRecreate: true, NoDependencies: true}}}, {StepId: task.Steps[2].ID, TimeoutSeconds: uint32(task.TimeoutSeconds), PrerequisiteStepId: task.Steps[1].ID, Payload: &agentpb.ExecutionStep_ComponentApply{ComponentApply: action}}}})
}

func (resolver *TaskPlanResolver) routeProviderFile(pin etcd.RouteProviderPin, projection etcd.EnvironmentComposeProjection) ([]byte, error) {
