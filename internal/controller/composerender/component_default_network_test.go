...
func TestComponentServiceRendersSelectedZoneGateway(t *testing.T) {
	...
	service, _, err := projectEnvironmentComponentService(
		environment, testcomponentrender.GeneratedEnvironmentService{
	...
	input.Identities.Networks = []composeidentity.Resource{{ID: zoneID, Name: "frontend"}}
	...
	if _, _, err := projectEnvironmentComponentService(environment, testcomponentrender.GeneratedEnvironmentService{ComponentID: componentID, Name: testComponentServiceName, Definition: definition}, image); err == nil {
