...
func finalizer(
	...
	return ControllerFinalizerInput{
		Finalizer:          controllerFinalizers[kind],
		TargetKind:         target,
		TargetID:           id,
		FixedInputRevision: revision,
		FixedInputDigest:   digest(salt + "/input"),
		BatchCount:         1,
	}
}
...
func node(
	target ActionTargetKind,
	id string,
	parentKind ActionTargetKind,
	parentID string,
	kind ActionKind,
	revision int64,
) MembershipNode {
	input := ProcedureInput{
		Kind:                ProcedureControllerFinalizer,
	...
	return MembershipNode{
		NodeID:         string(kind) + "\x00" + string(target) + "\x00" + id,
		TargetKind:     target,
		ID:             id,
		ActionKind:     kind,
...
func (e *fakeExecutor) Execute(_ context.Context, _ Operation, action Action) (AgentTerminalProof, error) {
	...
	return AgentTerminalProof{
		ChildOperationID:   action.Procedure.AgentChild.ChildOperationID,
		AttemptID:          "attempt-1",
		TaskID:             "tsk_child",
		...
		TerminalTaskDigest: digest("task"),
		Terminal:           AgentTerminalCompleted,
...
func (r *fakeRepository) ResolveDeletionTarget(
	_ context.Context,
	...
	if requested == TargetProject && target == TargetBackingService {
		return DeletionTargetResolution{}, errs.New(
...
func (r *fakeRepository) BeginDeletion(_ context.Context, b BeginDeletion) (BeginResult, error) {
	...
	r.operation = Operation{
		ID:              b.OperationID,
		TaskOperationID: b.TaskOperationIDCandidate,
		Kind:            b.OperationKind,
		TargetKind:      b.TargetKind,
		TargetID:        b.TargetID,
		TaskID:          b.TaskIDCandidate,
		Phase:           PhasePlanning,
	...
	for index, item := range planned {
		...
		if item.ProcedureInput.Kind == ProcedureAgentChild {
			...
			procedure.AgentChild = &AgentChildProcedure{
				ChildOperationID: fmt.Sprintf("op_0%025d", index+1),
				TaskType:         input.TaskType,
				TypedProcedure:   input.TypedProcedure,
		...
		actions = append(
			...
			Action{
				...
				PrerequisiteOrdinals: append([]int(nil), item.PrerequisiteOrdinals...),
				State:                ActionPending,
				Procedure:            procedure,
			},
		)
...
func (r *fakeRepository) AppendPlan(
	_ context.Context,
	_ Operation,
	actions []Action,
	start int,
...
func (r *fakeRepository) ConsumeAgentTerminal(
	_ context.Context,
	_ Operation,
	action Action,
...
func (r *fakeRepository) GetDeletionTaskIDAtRevision(
	_ context.Context,
	_ TargetKind,
	_ string,
	_ int64,
...
func service(snapshot FrozenMembership) (*Service, *fakeRepository, *fakeExecutor) {
	...
	return NewService(
		repo,
		executor,
		fixedIDs{task: "tsk_00000000000000000000000001"},
		fixedClock{now: time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC)},
	), repo, executor
}
...
func TestBuildPlanUsesStablePostorder(t *testing.T) {
	...
	snapshot := FrozenMembership{
		Revision:                42,
		RootRevision:            40,
		RootProcedureInput:      finalizer(ActionTenantFinalize, ActionTargetTenant, "tenant-1", 40, "tenant"),
		RootPrerequisiteNodeIDs: []string{project.NodeID},
		CoordinationEpoch:       3,
		Nodes:                   []MembershipNode{project, environment, entry},
	}
...
func TestBuildPlanRejectsReverseReference(t *testing.T) {
	_, err := BuildPlan(
		Operation{ID: "del", TargetKind: TargetProject, TargetID: "project-1"},
		FrozenMembership{
			Revision:           1,
			RootRevision:       1,
			RootProcedureInput: finalizer(ActionProjectFinalize, ActionTargetProject, "project-1", 1, "project"),
			CoordinationEpoch:  1,
			ReverseReferences: []ReverseReference{
				{
					SourceKind: ActionTargetRoute,
					SourceID:   "route-1",
					TargetKind: ActionTargetProject,
					TargetID:   "project-1",
				},
			},
		},
...
func TestBuildPlanUsesBackingFacadeRoot(t *testing.T) {
	operation := Operation{
		ID:         "del_backing",
		Kind:       OperationBackingDelete,
		TargetKind: TargetBackingService,
		TargetID:   "bks-1",
	}
	plan, err := BuildPlan(
		operation,
	...
	if root.Kind != ActionBackingServiceFinalize || root.TargetKind != ActionTargetBackingService ||
		root.TargetID != "bks-1" {
...
func TestBuildPlanAllowsMultipleActionsForOneTarget(t *testing.T) {
	...
	plan, err := BuildPlan(
		...
		FrozenMembership{
			Revision:                8,
			RootRevision:            8,
			RootProcedureInput:      finalizer(ActionProjectFinalize, ActionTargetProject, "project-1", 8, "project"),
			RootPrerequisiteNodeIDs: []string{finalize.NodeID},
			CoordinationEpoch:       1,
			Nodes:                   []MembershipNode{finalize, cleanup},
		},
	)
	if err != nil {
		t.Fatal(err)
...
func TestEnvironmentPlanRemovesDefaultComponentsBeforeRoot(t *testing.T) {
	...
	operation := Operation{
		ID:         "del_environment",
		Kind:       OperationEnvironmentDelete,
		TargetKind: TargetEnvironment,
		TargetID:   "env-1",
	}
	plan, err := BuildPlan(
		operation,
		FrozenMembership{
			...
			RootPrerequisiteNodeIDs: []string{cleanup.NodeID},
			CoordinationEpoch:       1,
			Nodes:                   []MembershipNode{cleanup, tunnel, caddy},
		},
	)
	if err != nil {
		t.Fatal(err)
	...
	if plan.Actions[0].Kind != ActionComponentRemove || plan.Actions[1].Kind != ActionComponentRemove ||
		plan.Actions[2].Kind != ActionEnvironmentAgentCleanup {
...
func TestDeleteIdempotencyAndConcurrency(t *testing.T) {
	svc, _, _ := service(
		FrozenMembership{
			Revision:           1,
			RootRevision:       1,
			RootProcedureInput: finalizer(ActionTenantFinalize, ActionTargetTenant, "tenant-1", 1, "tenant"),
			CoordinationEpoch:  1,
		},
	)
...
func TestExecuteStopsBeforeRootForAtomicTaskAcknowledgement(t *testing.T) {
	...
	accepted, err := svc.Delete(
		context.Background(),
		DeleteRequest{TargetKind: TargetEnvironment, TargetID: "env-1", IdempotencyKey: "key"},
	)
	if err != nil {
		t.Fatal(err)
	}
