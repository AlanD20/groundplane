...
func TestBlueprintTestStoreHistoricalGetManyAndReadCloning(t *testing.T) {
	...
	historical, err := store.GetMany(t.Context(), testkeyvalue.GetManyRequest{
	...
	for _, request := range []testkeyvalue.GetManyRequest{{}, {Keys: []string{"/items/one"}, Revision: -1}, {
...
func TestBlueprintTestStoreHistoricalRangeOrderingAndMore(t *testing.T) {
	...
	ascending, err := store.Range(t.Context(), testkeyvalue.RangeRequest{Prefix: "/items/", Limit: 2, Revision: view})
	if err != nil {
		t.Fatal(err)
	}
	...
	descending, err := store.Range(t.Context(), testkeyvalue.RangeRequest{
	...
	again, err := store.Range(t.Context(), testkeyvalue.RangeRequest{Prefix: "/items/", Limit: 3, Revision: view})
	if err != nil || len(again.Values) != 3 || string(again.Values[1].Value) != "/items/b" {
		t.Fatalf("cloned Range = %#v, %v", again, err)
	}
	for _, request := range []testkeyvalue.RangeRequest{
...
func TestBlueprintTestStoreCASFailureReadsAreAligned(t *testing.T) {
	...
	result, err := store.Transact(t.Context(), []testkeyvalue.Condition{
		{Key: "/missing", ModRevision: 0},
		{Key: "/prefix/", ModRevision: 0, Prefix: true},
		{Key: "/exact", ModRevision: 1},
	}, []testkeyvalue.Mutation{{Type: testkeyvalue.MutationPut, Key: "/should-not-exist", Value: []byte("no")}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBlueprintTestStoreTransactionsAreAtomic(t *testing.T) {
	...
	putValue := []byte("put")
	if _, err := store.Transact(t.Context(), nil, []testkeyvalue.Mutation{
		{Type: testkeyvalue.MutationPut, Key: "/atomic/one", Value: putValue},
		{Type: testkeyvalue.MutationType(99), Key: "/atomic/two"},
	...
	putValue[0] = 'X'
	result, err := store.Transact(t.Context(), nil, []testkeyvalue.Mutation{
		{Type: testkeyvalue.MutationPut, Key: "/atomic/one", Value: []byte("one")},
		{Type: testkeyvalue.MutationPut, Key: "/atomic/two", Value: []byte("two")},
	})
...
func TestBlueprintTestStoreDeleteAndVersionSemantics(t *testing.T) {
	...
	result, err := store.Transact(t.Context(), nil, []testkeyvalue.Mutation{{
		Type: testkeyvalue.MutationDelete, Key: "/tree/", Prefix: true,
	}})
	if err != nil || !result.Succeeded {
		t.Fatalf("prefix delete = %#v, %v", result, err)
	}
	page, err := store.Range(t.Context(), testkeyvalue.RangeRequest{Prefix: "/tree/", Limit: 1})
...
func TestBlueprintTestStoreFaultsAreBounded(t *testing.T) {
	t.Run("fail nth ordinary before commit", func(t *testing.T) {
		store := newBlueprintTestStore()
		store.setFault(
			func(conditions []testkeyvalue.Condition, mutations []testkeyvalue.Mutation) blueprintTestStoreFaultDirective {
				if len(mutations) > 0 {
	...
	t.Run("terminal compare loss", func(t *testing.T) {
		...
		store.setFault(func([]testkeyvalue.Condition, []testkeyvalue.Mutation) blueprintTestStoreFaultDirective {
			return blueprintTestStoreFaultDirective{
		...
		}}, []testkeyvalue.Mutation{{Type: testkeyvalue.MutationPut, Key: "/fault/terminal", Value: []byte("no")}}, true)
		...
		retry, err := store.transact(t.Context(), []testkeyvalue.Condition{{
			Key: "/fault/compared", ModRevision: result.FailureReads[0].ModRevision,
		}}, []testkeyvalue.Mutation{{Type: testkeyvalue.MutationPut, Key: "/fault/terminal", Value: []byte("yes")}}, true)
	...
	t.Run("terminal response lost after commit", func(t *testing.T) {
		...
		result, err := store.transact(t.Context(), nil, []testkeyvalue.Mutation{{
			Type: testkeyvalue.MutationPut, Key: "/fault/committed", Value: []byte("yes"),
		}}, true)
...
func keysOf(values []testkeyvalue.KeyValue) string {
	var joined []byte
