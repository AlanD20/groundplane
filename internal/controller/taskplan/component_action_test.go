...
func TestBuildComponentDisableExecutionPlanRestoresHostBeforeRemovingService(t *testing.T) {
	...
	artifact, err := composerender.RenderPlatformComponentCompose(composerender.PlatformComponentComposeInput{
	...
	plan, err := BuildComponentDisable(ComponentDisableInput{
	...
	if err != nil {
		t.Fatalf("BuildComponentDisable() error = %v", err)
	}
