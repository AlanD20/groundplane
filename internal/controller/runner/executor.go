...
func (executor *Executor) ExecuteRemove(ctx context.Context, task etcd.TaskRecord) error {
	...
	if task.Params[etcd.RunnerTenantIDParam] != current.Record.Desired.TenantID ||
		task.Params[etcd.RunnerOwnerKindParam] != string(current.Record.Desired.OwnerKind) ||
		task.Params[etcd.RunnerOwnerIDParam] != current.Record.Desired.OwnerID {
