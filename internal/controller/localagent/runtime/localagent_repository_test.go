...
func TestLocalAgentRepositoryAdapterTranslatesLifecycleWithoutAliasing(t *testing.T) {
	...
	adapter, err := NewRepository(repository, &fakeLocalAgentConfigIdempotency{})
	if err != nil {
...
func TestLocalAgentRepositoryAdapterRejectsUnknownDurablePhase(t *testing.T) {
	...
	repository := &fakeLocalAgentRecords{get: testkeyvalue.Versioned[testlocalagents.LocalAgentRecord]{
		Record: testlocalagents.LocalAgentRecord{Phase: "future"}, Revision: 1,
	}}
	adapter, err := NewRepository(repository, &fakeLocalAgentConfigIdempotency{})
	if err != nil {
		t.Fatalf("NewRepository() error = %v", err)
...
type fakeLocalAgentRecords struct {
	created          testlocalagents.LocalAgentRecord
	get              testkeyvalue.Versioned[testlocalagents.LocalAgentRecord]
	...
	getCalls         int
	replacement      testlocalagents.LocalAgentRecord
}
...
func (repository *fakeLocalAgentRecords) CreateSingleton(
	...
	return testkeyvalue.Versioned[testlocalagents.LocalAgentRecord]{Record: record, Revision: 11}, nil
}
...
func (repository *fakeLocalAgentRecords) UpdateConfigIdempotent(
	_ context.Context,
	current testkeyvalue.Versioned[testlocalagents.LocalAgentRecord],
	config testlocalagents.LocalAgentConfig,
	_ testidempotency.IdempotencyMarker,
) (testkeyvalue.Versioned[testlocalagents.LocalAgentRecord], etcd.IdempotencyTransactionResult, error) {
...
func (repository *fakeLocalAgentRecords) MarkReady(
	...
	record.Phase = testlocalagents.LocalAgentPhaseReady
	record.ReadyAt = readyAt
	return testkeyvalue.Versioned[testlocalagents.LocalAgentRecord]{Record: record, Revision: 12}, nil
}
...
func (repository *fakeLocalAgentRecords) ReplaceGeneration(
	...
	record.Image = image
	record.Generation++
	record.Phase = testlocalagents.LocalAgentPhaseUpdating
...
func (repository *fakeLocalAgentRecords) MarkReplacementReady(
	...
	record.Phase = testlocalagents.LocalAgentPhaseReady
	return testkeyvalue.Versioned[testlocalagents.LocalAgentRecord]{Record: record, Revision: 13}, nil
}
...
func (repository *fakeLocalAgentRecords) BeginDelete(
	...
	record := repository.created
	record.Phase = testlocalagents.LocalAgentPhaseDeleting
	record.TokenDigest = ""
	return testkeyvalue.Versioned[testlocalagents.LocalAgentRecord]{Record: record, Revision: 13}, nil
}
