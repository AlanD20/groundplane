...
type fakeStaleAgentReader struct {
	record testkeyvalue.Versioned[testlocalagents.LocalAgentRecord]
	err    error
}
...
func TestStaleAgentTaskMaintenanceUsesExactReadyBoundary(t *testing.T) {
	...
	reader := &fakeStaleAgentReader{
		record: testkeyvalue.Versioned[testlocalagents.LocalAgentRecord]{Record: testlocalagents.LocalAgentRecord{
			ID: runtimeAdapterAgentID, Generation: 7, Phase: testlocalagents.LocalAgentPhaseReady,
			Config: testlocalagents.LocalAgentConfig{PullIntervalSeconds: 10, MaxConcurrentTasks: 4},
		}},
	}
	...
	maintenance, err := NewStaleTaskMaintenance(reader, registry, tasks)
	if err != nil {
		t.Fatalf("NewStaleTaskMaintenance() error = %v", err)
	}
...
func TestStaleAgentTaskMaintenanceRequiresObservedMatchingReady(t *testing.T) {
	...
	reader := &fakeStaleAgentReader{
		record: testkeyvalue.Versioned[testlocalagents.LocalAgentRecord]{Record: testlocalagents.LocalAgentRecord{
			ID: runtimeAdapterAgentID, Generation: 7, Phase: testlocalagents.LocalAgentPhaseReady,
			Config: testlocalagents.LocalAgentConfig{PullIntervalSeconds: 5, MaxConcurrentTasks: 2},
		}},
	}
	registry := agentchannel.NewRegistry()
	tasks := &fakeStaleAgentTaskTimeout{}
