...
func TestNewSecretValueProtectorRequiresControllerKey(t *testing.T) {
	...
	protector, err := NewControllerKeyProtector(nil)
	if protector != nil {
		t.Fatal("NewControllerKeyProtector(nil) returned a protector")
	}
	assertSecretValueInternalWithoutText(t, err, "")
}
...
func mustSecretValueProtector(t *testing.T, key secretValueKey) *Protector {
	t.Helper()
	cipher := &secretValueCipher{key: key}
	protector, err := NewProtector(cipher, cipher)
	if err != nil {
