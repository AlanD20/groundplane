...
func (stub *recoveryPointEnvironmentReaderStub) GetEnvironment(
	...
	if stub.err != nil {
		return testkeyvalue.Versioned[testhierarchy.EnvironmentRecord]{}, stub.err
	}
	return testkeyvalue.Versioned[testhierarchy.EnvironmentRecord]{
		Record:       testhierarchy.EnvironmentRecord{ID: testRecoveryPointEnvironmentID},
		Revision:     5,
		ReadRevision: 5,
	}, nil
}
...
type recoveryPointRepositoryStub struct {
	requests []testbackupruntime.BackupRecoveryPointPageRequest
	pages    []testbackupruntime.BackupRecoveryPointPage
}

func (stub *recoveryPointRepositoryStub) ListVerifiedRecoveryPointsByEnvironment(
	_context context.Context,
	_environmentID string,
	request testbackupruntime.BackupRecoveryPointPageRequest,
) (testbackupruntime.BackupRecoveryPointPage, error) {
...
func TestRecoveryPointReadServiceUsesAuthenticatedStableBoundary(t *testing.T) {
	t.Parallel()
	points := &recoveryPointRepositoryStub{pages: []testbackupruntime.BackupRecoveryPointPage{
	...
	if len(points.requests) != 2 ||
		points.requests[0] != (testbackupruntime.BackupRecoveryPointPageRequest{Limit: 50}) ||
		points.requests[1] != (testbackupruntime.BackupRecoveryPointPageRequest{
