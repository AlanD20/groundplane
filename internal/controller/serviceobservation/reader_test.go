...
type readerAgents struct {
	agent     etcd.Versioned[etcd.LocalAgentRecord]
	calls     int
	err       error
	onRecheck func(*etcd.Versioned[etcd.LocalAgentRecord])
}

func (agents *readerAgents) GetSingleton(context.Context) (etcd.Versioned[etcd.LocalAgentRecord], error) {
	agents.calls++
	result := agents.agent
...
func TestObservationReaderFencesHostIdentityAndProjectsCompleteSnapshot(t *testing.T) {
	for _, test := range []struct {
		...
		{"updating", func(agents *readerAgents) { agents.agent.Record.Phase = etcd.LocalAgentPhaseUpdating }, false},
		{"changed enrollment", func(agents *readerAgents) {
			agents.onRecheck = func(current *etcd.Versioned[etcd.LocalAgentRecord]) { current.Revision++ }
		}, false},
		{"changed generation", func(agents *readerAgents) {
			agents.onRecheck = func(current *etcd.Versioned[etcd.LocalAgentRecord]) { current.Record.Generation++ }
		}, false},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			agents := &readerAgents{agent: etcd.Versioned[etcd.LocalAgentRecord]{
			...
			observer, err := New(
				&fakeServices{pages: []etcd.Page[etcd.ServiceRecord]{servicePage(60, fixture)}},
			...
			got := reader.ObserveServices(t.Context(), []etcd.Versioned[etcd.ServiceRecord]{fixture.service})
			if len(got) != 1 {
				t.Fatal(got)
			}
			if test.available {
			...
			if agents.agent.Record.Phase != etcd.LocalAgentPhaseReady || agents.err != nil {
				if channel.calls != 0 {
