...
func (executor *Executor) Execute(
	...
	if executor == nil || executor.engine == nil || ctx == nil {
		return errs.New(errs.KindInternal, "managed-config helper container is not configured")
	}
	...
	if created.ID == "" {
		return errs.New(errs.KindInternal, "managed-config helper create returned an empty container id")
	}
	...
	if err != nil {
		return operationError(ctx, "attach", err)
	}
	...
	if wait.Result == nil || wait.Error == nil {
		joinOutput()
		return errs.New(errs.KindInternal, "managed-config helper wait channels are not configured")
	}
	...
	if err := writeAll(attached.Conn, framed); err != nil {
		joinOutput()
		return operationError(ctx, "write request", err)
	}
	if err := attached.CloseWrite(); err != nil {
		joinOutput()
		return operationError(ctx, "close request", err)
	}
	...
	for waited == nil || output == nil {
		...
		case response, open := <-waitResults:
			if !open {
				...
				if waitErrors == nil && waited == nil {
					joinOutput()
					return errs.New(errs.KindInternal, "managed-config helper wait closed without a status")
				}
				continue
			}
			owned := response
		...
		case waitErr, open := <-waitErrors:
			...
			joinOutput()
			if waitErr == nil {
				return errs.New(errs.KindInternal, "managed-config helper wait returned an empty error")
			}
		...
		case read := <-outputDone:
			...
			if read.err != nil {
				attached.Close()
				return errs.Wrap(errs.KindInternal, read.err)
			}
		case <-ctx.Done():
	...
	attached.Close()
	if waited.Error != nil || waited.StatusCode != 0 || len(output.stdout) != 0 {
