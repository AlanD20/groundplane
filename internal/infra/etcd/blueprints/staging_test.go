...
func TestEnvironmentZoneAndRouteMutationAuditStageSealsEveryAuthoredField(t *testing.T) {
	...
	for index := range zoneFields {
		...
		if digest := mutationAuditDigestForTest(t, claim, projection, dependencyDigest, EnvironmentDesiredMutationAudit{Zone: &changed}); digest == zoneDigest {
	...
	for index := range routeFields {
		...
		if digest := mutationAuditDigestForTest(t, claim, projection, dependencyDigest, EnvironmentDesiredMutationAudit{Route: &changed}); digest == routeDigest {
