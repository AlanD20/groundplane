...
func TestManualScriptClosingResumesOriginalReport(t *testing.T) {
	for _, failAt := range []int{2, 3} {
		t.Run(string(rune('0'+failAt)), func(t *testing.T) {
			...
			manualScriptCleanupCheckpoints(t, scripts, assignment, execution, TaskStatusCompleted)
			...
			result := manualScriptTerminalResult(assignment, TaskStatusCompleted)
			...
			if err != nil || reportValue == nil || !report.matches(TaskStatusCompleted, result) ||
			...
			_, err = tasks.AcknowledgeTask(ctx, assignment.Assignment.Record.AgentID, 1,
				...
				manualScriptTerminalResult(assignment, TaskStatusFailed), terminalAt.Add(time.Second))
			if !isKind(err, errs.KindStateConflict) || store.revision != revision {
				t.Fatalf("conflicting report changed closing authority: %v", err)
			}
			_, err = tasks.AppendTaskEvent(ctx, TaskEventInput{Identity: TaskEventIdentity{
			...
			}, State: TaskEventStateRunning, Payload: []byte(`{"message":"late event"}`)}, terminalAt.Add(time.Second))
			if !isKind(err, errs.KindStateConflict) || store.revision != revision {
			...
			for _, key := range []string{scriptSourceRootKey(execution.OperationID), manualScriptClosingReportKey(assignment.Task.Record.ID)} {
