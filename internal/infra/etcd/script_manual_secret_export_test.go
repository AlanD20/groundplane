...
func (fixture *ExecutedArtifactFixture) CreateManualJourneySecret(t *testing.T, ciphertext []byte) string {
	...
	record, err := NewProjectSecretRecord(ids.New(ids.KindSecret), project.Record.ID,
		"manual-journey", core.SecretKindFile, "secrets/manual-value", fixture.Environment.Record.CreatedAt)
	if err != nil {
		t.Fatal(err)
	}
	digest := sha256.Sum256(ciphertext)
	if _, err := repository.CreateSecret(ctx, ProjectSecretOwner(project), record, SecretEncryptedValue{
...
func (fixture *ExecutedArtifactFixture) CheckManualJourneySecretDeletion(t *testing.T, secretID string, active bool) {
	...
	if _, err := fixture.Tasks.AcknowledgeControllerTask(ctx, task.ID, TaskStatusCompleted, at.Add(2*time.Second)); err != nil {
		t.Fatal(err)
	}
...
func assertManualJourneySecretSource(t *testing.T, store hierarchyStore, secret Versioned[SecretRecord]) {
	t.Helper()
	ctx := context.Background()
	conditions := secretScriptAbsenceConditions(secret.Record.Secret.ID)
	members, err := store.Range(ctx, RangeRequest{Prefix: conditions[1].Key, Limit: 2})
	if err != nil || members == nil || len(members.Values) != 1 || members.More {
		t.Fatal("manual Secret source membership coverage is inconsistent")
	}
	reference, err := decodeEnvelope[ScriptSourceReference](members.Values[0].Value, "script-source-reference")
	if err != nil {
		t.Fatal(err)
	}
	stored, err := store.Get(ctx, secretValueKey(secret.Record.Secret.ID))
	if err != nil || stored == nil || stored.Entry == nil {
		t.Fatal("manual Secret ciphertext source disappeared")
	}
	value, err := decodeSecretEncryptedValue(stored.Entry.Value)
	defer clear(value.Ciphertext)
	if err != nil || reference.Source != secretScriptSource(secret.Record.Secret.ID) ||
