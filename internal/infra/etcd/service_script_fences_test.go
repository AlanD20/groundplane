...
func TestServiceScriptAbsenceRequiresBothCountAndMembership(t *testing.T) {
	for _, state := range []string{"absent", "referenced", "count-only", "membership-only", "corrupt-count"} {
		t.Run(state, func(t *testing.T) {
			...
			source := testscriptsourcereference.SourceIdentity{
				Kind: testscriptsourcereference.SourceService, ServiceID: ids.New(ids.KindService),
			}
			...
			conditions, err := testscriptsourceevidence.PrepareServiceScriptAbsence(ctx, store, source.ServiceID, 0)
