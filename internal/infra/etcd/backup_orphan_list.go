...
func (repository *BackupRuntimeRepository) ListBackupOrphansByEnvironment(
...
) (backupruntime.BackupRuntimePage[backupruntime.BackupOrphanRecord], error) {
	if err := recordcodec.ValidateID(ids.KindEnvironment, environmentID); err != nil {
		return backupruntime.BackupRuntimePage[backupruntime.BackupOrphanRecord]{}, err
	}
	...
	if index == nil || index.ReadRevision <= 0 {
		return backupruntime.BackupRuntimePage[backupruntime.BackupOrphanRecord]{}, errs.New(
			errs.KindInternal,
	...
	for position, item := range index.Values {
		...
		if recordcodec.ValidateID(ids.KindRecoveryPoint, pointID) != nil {
			return backupruntime.BackupRuntimePage[backupruntime.BackupOrphanRecord]{}, backupruntime.CorruptBackupRuntimeRecord()
		}
	...
	page := backupruntime.BackupRuntimePage[backupruntime.BackupOrphanRecord]{Revision: index.ReadRevision}
	if len(keys) == 0 {
		return page, nil
	}
