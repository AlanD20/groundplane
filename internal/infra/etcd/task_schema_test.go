...
func TestTaskJournalSchemaGateInitializesOnlyAnEmptyTaskCollection(t *testing.T) {
	...
	marker, err := store.Get(ctx, testtaskjournal.TaskJournalSchemaKey)
	if err != nil || marker.Entry == nil || string(marker.Entry.Value) != testtaskjournal.TaskJournalSchemaValue {
...
func TestTaskJournalSchemaGateRejectsLegacyMalformedAndFailedCAS(t *testing.T) {
	...
	tests := map[string]func(*testing.T, *memoryTaskStore) taskRepositoryStore{
		"legacy tasks": func(t *testing.T, store *memoryTaskStore) taskRepositoryStore {
			seedTaskRepositoryValue(t, store, testtaskjournal.TaskStorageKey(ids.New(ids.KindTask)), []byte("legacy"))
			return store
		},
		"malformed marker": func(t *testing.T, store *memoryTaskStore) taskRepositoryStore {
			seedTaskRepositoryValue(t, store, testtaskjournal.TaskJournalSchemaKey, []byte("not-v1"))
			return store
		},
	}
...
func TestTaskJournalSchemaGateRejectsTaskCreatedDuringInitialization(t *testing.T) {
	...
	seedTaskRepositoryValue(
		...
		testtaskjournal.TaskStorageKey(ids.New(ids.KindTask)),
		[]byte("racing-task"),
	)
	close(store.proceed)
...
func (store *taskSchemaRaceStore) Transact(
	...
	case <-store.proceed:
	case <-ctx.Done():
		return testkeyvalue.TransactionResult{}, ctx.Err()
	}
