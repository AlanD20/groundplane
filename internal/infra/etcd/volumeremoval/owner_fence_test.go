...
// Volume owner disappears or belongs to another operation, including at commit.
func TestVolumeRemovalExecutionRequiresExactOwner(t *testing.T) {
	for _, action := range []string{"detach", "begin", "redeliver", "retry"} {
		...
		for _, change := range changes {
			t.Run(action+"/"+change, func(t *testing.T) {
				...
				if action == "begin" || action == "redeliver" {
					if _, err := repository.MarkConsumersDetached(ctx, assignment, runtime.CreatedAt.Add(4*time.Second)); err != nil {
						t.Fatal(err)
					}
				}
				if action == "redeliver" {
					if _, _, err := repository.BeginPathCall(ctx, assignment, runtime.CreatedAt.Add(5*time.Second)); err != nil {
						t.Fatal(err)
					}
				}
				...
				switch action {
				case "detach":
					_, err = repository.MarkConsumersDetached(ctx, assignment, runtime.CreatedAt.Add(6*time.Second))
