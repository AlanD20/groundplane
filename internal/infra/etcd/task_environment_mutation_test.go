...
func TestOrdinaryTaskEnvironmentMutationUsesFixedRevisionAndFailsClosed(t *testing.T) {
	...
	task.Params = map[string]string{TaskServiceEnvironmentParam: environment.Record.ID}
	...
	conflict := binding.classify(
		result.Revision,
		result.FailureReads,
		func(int64, []*KeyValue) error { return errs.New(errs.KindStateConflict, "domain changed") },
	)
...
func TestOrdinaryTaskEnvironmentMutationRejectsHeldLockAndOversizedBatch(t *testing.T) {
	...
	result, err := store.Transact(context.Background(), nil, []Mutation{{
		Type: MutationDelete, Key: environmentOperationLockKey(environment.Record.ID),
	}})
	if err != nil || !result.Succeeded {
