...
func TestHierarchyEnvironmentPoolReplacementIsAtomicAndReplays(t *testing.T) {
	...
	epoch, err := store.Get(context.Background(), environmentMutationEpochKey(current.Record.ID))
	...
	zones, err := store.Get(context.Background(), zonePoolRegistryKey(current.Record.ID))
...
func TestHierarchyEnvironmentPoolReplacementLosesConcurrentZoneReservation(t *testing.T) {
	...
	racing.key = zonePoolRegistryKey(current.Record.ID)
...
func TestHierarchyEnvironmentPoolReplacementLosesGlobalRegistryCAS(t *testing.T) {
	...
	racing := &environmentPoolMutationRaceStore{memoryHierarchyStore: base}
	repository, _, current, _ := environmentPoolMutationFixtureWithStore(t, racing, nil)
	globalValue, err := encodeEnvelope("environment_pool_registry", EnvironmentPoolRegistry{
	...
	racing.key = environmentPoolRegistryKey
...
func TestHierarchyEnvironmentPoolReplacementLosesTenantDeletionFence(t *testing.T) {
	...
	racing := &environmentPoolMutationRaceStore{memoryHierarchyStore: base}
	repository, _, current, _ := environmentPoolMutationFixtureWithStore(t, racing, nil)
	racing.key = deletionTombstoneKey("tenant", hierarchyTestID(ids.KindTenant, 705))
...
) (*HierarchyRepository, *memoryHierarchyStore, Versioned[EnvironmentRecord], int64) {
	t.Helper()
...
func environmentPoolMutationFixtureWithStore(
	...
	tenant := TenantRecord{ID: hierarchyTestID(ids.KindTenant, 705), Slug: "acme", Name: "Acme"}
	...
	seeded, err := base.Transact(context.Background(), nil, []Mutation{
		{Type: MutationPut, Key: environmentPoolRegistryKey, Value: globalValue},
		{Type: MutationPut, Key: zonePoolRegistryKey(record.ID), Value: zoneValue},
	})
...
func environmentPoolMutationZoneValue(t *testing.T, reservations map[string]string) []byte {
	t.Helper()
	value, err := encodeEnvelope("zone_pool_registry", zonePoolRegistry{Reservations: reservations})
	if err != nil {
...
func environmentPoolMutationTestMarker(environmentID string, key string) IdempotencyMarker {
