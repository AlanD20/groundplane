...
func (repository *BackupPolicyRepository) loadBackupPolicyReplacementBase(
	...
	tenantID string,
	now time.Time,
) (backuppolicymutations.ReplacementCandidate, bool, error) {
	...
	if err != nil {
		return backuppolicymutations.ReplacementCandidate{}, false, err
	}
	if result == nil || result.ReadRevision <= 0 || len(result.Values) != 11 {
		return backuppolicymutations.ReplacementCandidate{}, false, errs.New(
			errs.KindInternal,
	...
	if err != nil || environment.ID != input.EnvironmentID || environment.ProjectID != projectID {
		return backuppolicymutations.ReplacementCandidate{}, false, recordcodec.CorruptRecord()
	}
	...
	if result.Values[1] == nil {
		return backuppolicymutations.ReplacementCandidate{}, false, errs.New(errs.KindProjectNotFound, "project was not found")
	}
	...
	candidate := backuppolicymutations.ReplacementCandidate{
		...
		Sources: make([]backuppolicymutations.SourceEvidence, len(input.Sources)),
	}
	...
	if result.Values[10] != nil {
		...
		if coordinationErr != nil || decoded.EnvironmentID != input.EnvironmentID {
			return backuppolicymutations.ReplacementCandidate{}, false, coordinationrecord.CorruptRecord()
		}
	...
	if (result.Values[5] == nil) != (result.Values[6] == nil) {
		return backuppolicymutations.ReplacementCandidate{}, false, backuppolicy.CorruptBackupKey()
	}
