...
func TestEntryCreateIdempotentCommitsMetadataGenerationAndMarker(t *testing.T) {
	...
	record, err := testentries.NewRecord(environment.Record.ID, entry, generationID)
	if err != nil {
	...
	result, err := repository.CreateEntryIdempotent(
		...
		project,
		record,
		testentries.EntryValueGeneration{Plain: &generation},
		marker,
	)
	...
	if err != nil || !testentries.EqualRecord(stored.Record, record) {
...
func TestEntryReplayTargetAcceptsOnlyStableEntryIdentity(t *testing.T) {
	...
	if _, err := testidempotency.IdempotencyReplayTargetKey(
