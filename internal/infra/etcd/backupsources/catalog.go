...
func EnsureBackupSource(
	...
	for attempt := 0; attempt < maximumBackupSourceEnsureAttempts; attempt++ {
		existing, found, err := getBackupSourceByIdentity(ctx, store,
		...
		creation, err := loadBackupSourceCreationEvidence(ctx, store,
			environment,
			project,
			kind,
			targetID,
		...
		record := backuppolicy.BackupSourceRecord{
			...
			Kind: kind, TargetID: targetID, CreatedAt: now().UTC(),
		}
		created, err := createBackupSource(ctx, store, creation, record)
		if err == nil {
			return created, nil
		}
...
func createBackupSource(
	...
	result, err := store.Transact(ctx, []etcdstore.Condition{
...
func loadBackupSourceCreationEvidence(
	ctx context.Context,
	store catalogStore,
	environment etcdstore.Versioned[hierarchyrecord.EnvironmentRecord],
	project etcdstore.Versioned[hierarchyrecord.ProjectRecord],
...
func getBackupSourceByIdentity(
	ctx context.Context,
	store catalogStore,
	environmentID string,
	kind core.BackupSourceKind,
	...
	index, err := store.Get(ctx, backuppolicy.BackupSourceIdentityKey(environmentID, kind, targetID))
	if err != nil {
	...
	stored, err := store.GetMany(ctx, etcdstore.GetManyRequest{
		Keys: []string{
			backuppolicy.BackupSourceKey(sourceID),
