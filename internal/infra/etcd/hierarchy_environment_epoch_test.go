...
func TestHierarchyEnvironmentCreationRejectsExistingMutationEpoch(t *testing.T) {
	...
	project, err := repository.CreateProject(ctx, testhierarchy.ProjectRecord{
		...
		Slug: "epoch-project", Name: "Epoch Project", Kind: testhierarchy.ProjectKindTenant,
	})
	if err != nil {
	...
	seeded, err := store.Transact(
		ctx,
		[]testkeyvalue.Condition{{Key: testhierarchy.EnvironmentMutationEpochKey(environmentID)}},
	...
	epoch, err := store.Get(ctx, testhierarchy.EnvironmentMutationEpochKey(environmentID))
