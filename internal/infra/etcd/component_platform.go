...
func (repository *ComponentRepository) CreatePlatformComponent(ctx context.Context, record ComponentRecord) (Versioned[ComponentRecord], error) {
	...
	result, err := repository.store.Transact(ctx, []Condition{
		{Key: componentKey(record.Desired.ID)}, {Key: platformComponentOwnerKey(record.Desired.ID)},
		{Key: platformComponentKindKey(record.Desired.Kind)}, {Key: deletionTombstoneKey("component", record.Desired.ID)},
	}, []Mutation{
	...
	if !result.Succeeded {
		return Versioned[ComponentRecord]{}, errs.New(errs.KindStateConflict, "platform Component stable identity is already in use")
	}
	return Versioned[ComponentRecord]{Record: record, Revision: result.Revision, ReadRevision: result.Revision}, nil
}

func (repository *ComponentRepository) ListPlatformComponents(ctx context.Context, request PageRequest) (Page[ComponentRecord], error) {
...
func (repository *ComponentRepository) ReplacePlatformDesired(ctx context.Context, current Versioned[ComponentRecord], desired core.Component) (Versioned[ComponentRecord], error) {
...
func (repository *ComponentRepository) ReplacePlatformRuntime(ctx context.Context, current Versioned[ComponentRecord], generatedServices []string, pinnedIPv4 string, healthy bool) (Versioned[ComponentRecord], error) {
...
func (repository *ComponentRepository) replacePlatform(ctx context.Context, current Versioned[ComponentRecord], replacement ComponentRecord) (Versioned[ComponentRecord], error) {
	...
	if current.Revision <= 0 || current.ReadRevision < current.Revision || current.Record.Desired.ID != replacement.Desired.ID {
		return Versioned[ComponentRecord]{}, errs.New(errs.KindValidationFailed, "platform Component version metadata is invalid")
	}
	indexes, err := repository.store.GetMany(ctx, GetManyRequest{Keys: []string{platformComponentOwnerKey(current.Record.Desired.ID), platformComponentKindKey(current.Record.Desired.Kind)}, Revision: current.ReadRevision})
	if err != nil {
		return Versioned[ComponentRecord]{}, err
	}
	if indexes == nil || len(indexes.Values) != 2 || indexes.Values[0] == nil || indexes.Values[1] == nil || string(indexes.Values[0].Value) != current.Record.Desired.ID || string(indexes.Values[1].Value) != current.Record.Desired.ID {
		return Versioned[ComponentRecord]{}, errs.New(errs.KindInternal, "platform Component indexes are missing or corrupt")
	}
	...
	return Versioned[ComponentRecord]{Record: replacement, Revision: result.Revision, ReadRevision: result.Revision}, nil
}
...
func validateComponentObservation(record ComponentObservationRecord) error {
	if ids.Validate(ids.KindComponent, record.ComponentID) != nil || ids.Validate(ids.KindService, record.ServiceID) != nil || ids.Validate(ids.KindAgent, record.AgentID) != nil || record.DesiredGeneration == 0 || record.RenderGeneration == 0 || record.AgentGeneration == 0 || record.OwnershipGeneration == 0 || !validMarkerTime(record.ObservedAt) {
	...
	} else if record.CorefileSHA256 != "" || record.InputSHA256 != "" {
...
func (repository *ComponentRepository) PutPlatformComponentObservation(ctx context.Context, record ComponentObservationRecord, expectedComponentRevision int64, expectedObservationRevision int64) (Versioned[ComponentObservationRecord], error) {
	if err := validateContext(ctx); err != nil {
	...
	if component.Record.Desired.Owner != core.ComponentOwnerPlatform || component.Record.Desired.OwnerID != "" || component.Revision != expectedComponentRevision {
	...
	if current == nil || revisionChanged(current.Entry, expectedObservationRevision) {
		return Versioned[ComponentObservationRecord]{}, stateConflict("platform component observation", record.ComponentID)
	}
	...
	defer clear(value)
	result, err := repository.store.Transact(ctx, []Condition{{Key: componentKey(record.ComponentID), ModRevision: expectedComponentRevision}, {Key: componentObservationKey(record.ComponentID), ModRevision: expectedObservationRevision}, {Key: deletionTombstoneKey("component", record.ComponentID)}}, []Mutation{{Type: MutationPut, Key: componentObservationKey(record.ComponentID), Value: value}})
	if err != nil {
		return Versioned[ComponentObservationRecord]{}, err
	}
	if !result.Succeeded {
	...
	return Versioned[ComponentObservationRecord]{Record: record, Revision: result.Revision, ReadRevision: result.Revision}, nil
}
func (repository *ComponentRepository) GetPlatformComponentObservation(ctx context.Context, componentID string) (Versioned[ComponentObservationRecord], bool, error) {
	if err := validateContext(ctx); err != nil {
	...
	if result == nil {
		return Versioned[ComponentObservationRecord]{}, false, errs.New(errs.KindInternal, "Component observation read is empty")
	}
	if result.Entry == nil {
	...
	if err != nil || record.ComponentID != componentID {
		return Versioned[ComponentObservationRecord]{}, false, errs.New(errs.KindInternal, "Component observation is corrupt")
	}
	return Versioned[ComponentObservationRecord]{Record: record, Revision: result.Entry.ModRevision, ReadRevision: result.ReadRevision}, true, nil
}
