...
func TestManualScriptRetentionPruneReleasesRetrySources(t *testing.T) {
	...
	if err != nil || closed.Record.State != testscriptexecutions.ScriptExecutionCleanupProven ||
		...
		closed.Record.Outcome.Reason != testscriptexecutions.ScriptOutcomeExpiryBeforeStart {
	...
	for _, failAt := range []int{2, 3} {
		t.Run(strconv.Itoa(failAt), func(t *testing.T) {
			...
			terminal, err := tasks.AcknowledgeTask(
				...
				1,
				assignment.Task.Record.ID,
				assignment.Assignment.Record.AssignmentID, testtaskjournal.TaskStatusFailed, manualScriptTerminalResult(
					assignment, testtaskjournal.TaskStatusFailed,
				),
				assignment.Task.Record.CreatedAt.Add(20*time.Second),
			)
			if err != nil {
			...
			taskBefore := store.valueAt(testtaskjournal.TaskStorageKey(terminal.Record.ID), store.revision)
			indexBefore := store.valueAt(
				testtaskjournal.TaskRetentionIndexKey(terminal.Record.ID, *terminal.Record.RetainUntil),
				store.revision,
			)
			...
			for _, before := range []*testkeyvalue.KeyValue{taskBefore, indexBefore} {
			...
			if err != nil || root.ReleasePath != testscriptsourceevidence.ScriptSourceReleaseRetryExpiry ||
				root.RetryDisposition != testscriptsourcereference.RetryDispositionExpired || root.RetryExpiresAt == nil ||
			...
			if _, _, err := authority.ReleaseNext(ctx, execution.OperationID,
				[]testkeyvalue.Condition{{Key: taskBefore.Key, ModRevision: taskBefore.ModRevision}}); err == nil || store.revision != beforeWrongPath {
...
func TestManualScriptRetentionSkipsLiveRetryWithoutDroppingIndex(t *testing.T) {
	...
	if _, err := tasks.RetryTask(ctx, terminal.Record.ID, retryID, testtaskjournal.TaskActorOperator, pendingRetryMarker(terminal.Record, retryID, at.Add(time.Second), "live-manual-retry")); err != nil {
		t.Fatal(err)
	}
	unrelated := validTaskRecord(at.Add(time.Minute))
	unrelated.Type = testtaskjournal.TaskUpdate
	...
	for _, key := range []string{testtaskjournal.TaskStorageKey(terminal.Record.ID), testtaskjournal.TaskRetentionIndexKey(terminal.Record.ID, *terminal.Record.RetainUntil)} {
