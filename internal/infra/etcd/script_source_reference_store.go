...
func (adapter *scriptSourceReferenceStore) GetMany(ctx context.Context, keys []string, revision int64) (*ref.GetManyResult, error) {
	...
	for index, value := range read.Values {
		if value != nil {
			result.Values[index] = &ref.KeyValue{Key: value.Key, Value: append([]byte(nil), value.Value...), ModRevision: value.ModRevision}
		}
	}
	return result, nil
}

func (adapter *scriptSourceReferenceStore) Range(ctx context.Context, prefix string, limit int64) (*ref.RangeResult, error) {
	read, err := adapter.store.Range(ctx, RangeRequest{Prefix: prefix, Limit: limit})
	if err != nil || read == nil {
		return nil, err
	}
	result := &ref.RangeResult{Values: make([]ref.KeyValue, len(read.Values)), ReadRevision: read.ReadRevision, More: read.More}
	for index, value := range read.Values {
		result.Values[index] = ref.KeyValue{Key: value.Key, Value: append([]byte(nil), value.Value...), ModRevision: value.ModRevision}
	}
	return result, nil
...
func (adapter *scriptSourceReferenceStore) Transact(ctx context.Context, conditions []ref.Condition, mutations []ref.Mutation) (ref.TransactionResult, error) {
	result, err := adapter.store.Transact(ctx, convertScriptSourceConditions(conditions), convertScriptSourceMutations(mutations))
	return ref.TransactionResult{Succeeded: result.Succeeded, Revision: result.Revision}, err
}

func (adapter *scriptSourceReferenceStore) AdjustScriptPrimary(value []byte, source ref.SourceIdentity, delta int64) ([]byte, error) {
...
func convertScriptSourceMutations(input []ref.Mutation) []Mutation {
	...
	for index, mutation := range input {
		result[index] = Mutation{Type: MutationType(mutation.Type), Key: mutation.Key, Value: append([]byte(nil), mutation.Value...), Prefix: mutation.Prefix}
	}
	return result
}
