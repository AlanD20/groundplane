...
func (repository *Preparer) PrepareReleaseGroupBlueprintMutation(
	ctx context.Context,
	...
	for _, id := range currentIDs {
		...
		conditions = append(
			...
			etcdstore.Condition{
				Key:         ReleaseGroupOwnerKey(environmentID, id),
			...
			etcdstore.Condition{
				Key: deletionrecord.TombstoneKey(string(deletionrecord.DeletionTargetReleaseGroup), id),
			},
		)
	...
	for _, id := range desiredIDs {
		...
		mutations = append(
			...
			etcdstore.Mutation{
				Type:  etcdstore.MutationPut,
				Key:   ReleaseGroupNameKey(environmentID, group.Name),
				Value: []byte(id),
			},
		)
	}
