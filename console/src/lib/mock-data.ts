...
function storefrontEnv(env: 'staging' | 'production'): Environment {
  ...
  return {
    ...
    scripts: [
      { id: fixtureId('scr', `migrate-${env}`), environmentId: envId, slug: 'migrate', serviceId: svcId('app-api', env), service: 'app-api', when: 'pre-deploy', body: 'php artisan migrate --force --isolated', origin: 'blueprint', reconciliationKey: 'migrate', activeGeneration: 1 },
      { id: fixtureId('scr', `preflight-${env}`), environmentId: envId, slug: 'realtime-preflight', serviceId: svcId('app-websocket', env), service: 'app-websocket', when: 'post-deploy', body: 'php artisan realtime:preflight', origin: 'blueprint', reconciliationKey: 'realtime-preflight', activeGeneration: 1 },
      { id: fixtureId('scr', `cache-${env}`), environmentId: envId, slug: 'clear-cache', serviceId: svcId('app-api', env), service: 'app-api', when: 'manual', body: 'php artisan optimize:clear\nphp artisan config:cache', origin: 'api', activeGeneration: 1 },
      { id: fixtureId('scr', `onfail-${env}`), environmentId: envId, slug: 'notify-failure', serviceId: svcId('app-api', env), service: 'app-api', when: 'on-failure', body: 'php artisan groundplane:notify --channel=ops "deploy failed"', origin: 'blueprint', reconciliationKey: 'notify-failure', activeGeneration: 1 },
    ],
    backup: {
      enabled: true,
      frequency: '*-*-* 03:15:00',
...
function sampleSiteEnv(env: 'staging' | 'production'): Environment {
  ...
  return {
    ...
    scripts: [
      { id: fixtureId('scr', `sample-site-migrate-${env}`), environmentId: envId, slug: 'migrate', serviceId: svcId('cms', env), service: 'cms', when: 'pre-deploy', body: 'node ./bin/migrate.js', origin: 'blueprint', reconciliationKey: 'migrate', activeGeneration: 1 },
      { id: fixtureId('scr', `sample-site-seed-${env}`), environmentId: envId, slug: 'seed-content', serviceId: svcId('cms', env), service: 'cms', when: 'manual', body: 'node ./bin/seed.js --content', origin: 'api', activeGeneration: 1 },
    ],
    backup: {
