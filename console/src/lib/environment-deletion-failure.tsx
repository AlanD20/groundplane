...
export function EnvironmentDeletionFailureNotice({ failure, onRetry, retryLabel }: EnvironmentDeletionFailureNoticeProps) {
  const [retrying, setRetrying] = useState(false)
  const [error, setError] = useState<string | null>(null)

  return (
    <div className="flex flex-col gap-2 rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-2">
      <p className="text-xs text-destructive" role="alert">{failure.message}</p>
      ...
      <Button
        ...
        onClick={async () => {
          ...
          try {
            ...
            setError(reason instanceof Error ? reason.message : 'Unable to retry Environment deletion')
          } finally {
            setRetrying(false)
          }
        }}
