...
test('plaintext is neither fixture-backed nor retained by drawers', () => { assert.doesNotMatch(fixtures, /platformSecrets|SENTRY_DSN/); for (const page of [projectPage, platformPage]) { assert.match(page, /if \(!next\) clearDraft\(\)/); assert.match(page, /catch \(cause\) \{\s*setValue\(''\)/) }; assert.match(ownerList, /<RevealValue loadValue=\{\(\) => onReveal\(secret\)\}/) })
