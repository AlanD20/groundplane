...
import type { Script, ScriptInput, ScriptPatch } from './script-types'
import { scriptExecutionFromAPI, scriptExecutionToAPI } from './script-execution-api.ts'
...
export function scriptCreateToAPI(environmentId: string, serviceId: string, input: ScriptInput): ScriptCreateRequest {
  return {
    environment_id: environmentId, service_id: serviceId, slug: input.slug,
    script: input.body, when: input.when, order: input.order, execution: scriptExecutionToAPI(input.execution),
  }
}
...
export function scriptPatchToAPI(patch: ScriptPatch): ScriptEditRequest {
  ...
  if (patch.execution !== undefined) body.execution = scriptExecutionToAPI(patch.execution)
  return body
}
