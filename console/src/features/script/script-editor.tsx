...
import { scriptExecutionError } from "@/features/script/execution";
...
export function ScriptEditor({
  ...
  const [service, setService] = useState(
    script?.service ?? env.services[0]?.name ?? "",
  );
  ...
  const [execution, setExecution] = useState<ScriptExecution>(
    script?.execution ?? { mode: "inherited" },
  );
  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          setSubmitError(
            error instanceof Error ? error.message : "Script mutation failed",
          );
        } finally {
          setSubmitting(false);
        }
    ...
    >
      <DialogHeader>
        <DialogTitle>
          {script ? `Edit script · ${script.slug}` : `Add script · ${env.name}`}
        </DialogTitle>
      </DialogHeader>
      <fieldset disabled={submitting} className="flex min-w-0 flex-col gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-1.5">
            ...
            <Select
              id="script-when"
              value={when}
              options={hooks.map((hook) => ({ value: hook, label: hook }))}
              onValueChange={(value) => {
                const hook = hooks.find((hook) => hook === value);
                if (hook) setWhen(hook);
              }}
            />
          </div>
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              ...
              aria-invalid={parsedOrder === undefined}
              aria-describedby="script-order-help"
            />
          </div>
        </div>
        <p id="script-order-help" className="text-xs text-muted-foreground">
          0–65535. Lower numbers run first within this Service and phase; ties
          use the Script slug. Service dependencies and Release Group order are
          unchanged. Manual runs ignore this field.
        </p>
        {parsedOrder === undefined && (
        ...
        <p className="text-xs text-muted-foreground">
          Hooks run automatically for selected deploys and rollbacks. Saving
          replaces the complete execution choice; published runs keep their
          captured context. Resource availability is checked before a run is
          published.
        </p>
      </fieldset>
      {submitError && (
      ...
      <DialogFooter>
        ...
        <Button type="submit" disabled={submitting || !valid}>
          {submitting ? "Saving…" : script ? "Save script" : "Add script"}
        </Button>
      </DialogFooter>
    </form>
  );
