...
export function BlueprintWorkspace({
  ...
  const [snapshot, setSnapshot] = useState<BlueprintDocumentResponse | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Blueprint could not be loaded.')
    } finally {
      setLoading(false)
    }
  ...
  function exportDocument() {
    if (!snapshot) return
    const url = URL.createObjectURL(new Blob([draft], { type: 'application/yaml;charset=utf-8' }))
  ...
  async function reviewDraft() {
    ...
    try {
      ...
      const validation = await store.validateBlueprint(environment.id, request, snapshot.revision)
      setPrepared({ request, validation })
      setDispatched(false)
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Blueprint validation failed.')
    } finally {
      setWorking(false)
    }
  }
      ...
      <Card>
        <CardHeader className="gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Canonical authored input only. Runtime ids, generated paths, observations, and secret values are excluded.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <BlueprintApplyAction environment={environment} workspace={workspace} />
            ...
            />
            <Button variant="outline" size="sm" disabled={!snapshot} onClick={exportDocument}>
              <Download /> Export
            </Button>
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" disabled={loading || working} onClick={() => void load()}>
                <RefreshCw /> Refresh
              </Button>
              {!editing && (
                <Button size="sm" disabled={!snapshot} onClick={() => setEditing(true)}>
                  <Pencil /> Edit
                </Button>
              )}
              {editing && (
                <>
                  ...
                  <Button size="sm" disabled={working || !draft.trim()} onClick={() => void reviewDraft()}>
          ...
          <p className="text-xs text-muted-foreground">
            Omitted existing resources are retained. Destruction remains available only through each resource's explicit
            Remove action.
          </p>
        </CardContent>
      </Card>
