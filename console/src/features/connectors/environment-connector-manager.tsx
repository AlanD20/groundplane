...
import { newConnectorMutationIntent, type ConnectorMutationIntent } from '@/lib/connector-intent'
import { isNoResponseTransportUncertainty, useStore } from '@/lib/store'
import type { Connector, ConnectorCreateInput, ConnectorCredential, ConnectorCredentialInput, Environment, ReusableSecret } from '@/lib/types'
...
export function EnvironmentConnectorManager({
  env,
}: {
  env: Environment
}) {
  ...
  const tenantSlug = store.tenants.find((tenant) => tenant.id === project?.tenantId)?.slug ?? project?.tenantId ?? ''
  const projectSlug = project?.slug ?? env.projectId
  const connectors = store.connectors.filter((connector) => connector.scopeRef === env.id)
  const secretOptions = store.reusableSecrets.filter(
    (secret) =>
      secret.kind === 'env' &&
      (secret.scope === 'platform' || (secret.scope === 'project' && secret.projectId === env.projectId)),
  )
  const backupPolicyState = store.getBackupPolicyState(env.id)
  const backupPolicy = backupPolicyState.policy
  const policyAuthoritative = backupPolicyState.loaded
    && !backupPolicyState.loading
    && !backupPolicyState.loadError
  const activeConnector = connectors.find((connector) => connector.id === backupPolicy.connectorId)
  ...
  const clearRemoveDispatchIntent = () => {
    setRemoveIntent((current) => current ? { ...current, intent: null } : null)
  }
  ...
  return (
    <>
      <Card>
        <CardHeader className="flex-row items-center justify-between gap-3">
          <div className="flex flex-col gap-1">
            <CardTitle className="flex items-center gap-2">
              <Plug className="size-4 text-muted-foreground" /> Backup connectors
            </CardTitle>
            <p className="text-xs text-muted-foreground">
              Destinations owned only by this environment. Connectors never inherit from project or platform scope.
            </p>
          </div>
        ...
        <CardContent className="flex flex-col gap-2">
          {store.connectorsLoading ? (
            <p className="rounded-lg border border-border bg-surface px-3 py-4 text-sm text-muted-foreground" role="status">
              Loading connectors...
            </p>
          ) : store.connectorError ? (
            <p className="rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-3 text-sm text-destructive" role="alert">
              {store.connectorError}
            </p>
          ) : connectors.length === 0 ? (
            <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed border-border px-4 py-8 text-center">
              ...
              <div>
                <p className="text-sm font-medium">No connector for this environment</p>
                <p className="text-xs text-muted-foreground">Create one before enabling backups.</p>
              </div>
            </div>
          ) : (
            connectors.map((connector) => {
              const active = policyAuthoritative && backupPolicy.enabled && connector.id === activeConnector?.id
              return (
                <div
                  key={connector.id}
                  ...
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="flex min-w-0 flex-col gap-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-mono text-sm font-medium">{connector.name}</span>
                        {active && <Badge variant="default">policy target</Badge>}
                        <Badge variant="outline">{connector.kind}</Badge>
                      </div>
                      <span className="break-all font-mono text-xs text-muted-foreground">
                        s3://{connector.bucket}/{normalizedPrefix(connector.prefix)}
                      </span>
                      <span className="truncate text-xs text-muted-foreground">{connector.endpoint}</span>
                    </div>
                  </div>
                  <div className="flex shrink-0 items-center justify-end gap-1">
                    <CopyButton value={`s3://${connector.bucket}/${normalizedPrefix(connector.prefix)}`} />
                    <Button
                      variant="ghost"
                      size="icon-sm"
                    ...
                    <Button
                      ...
                      onClick={() => policyAuthoritative && !active && beginRemove(connector)}
          ...
          {policyAuthoritative
            ...
            <p className="rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              Backup policy references missing connector <span className="font-mono">{backupPolicy.connectorId}</span>. Select an
              environment-owned connector before enabling or running backups.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <TaskRunnerDialog
        ...
        steps={[{ label: 'Remove metadata and encrypted credentials', state: 'pending' }]}
        onDispatch={() => {
          if (!removing) return Promise.reject(new Error('No Connector selected'))
          if (removing.scopeRef !== env.id) {
            cancelRemove()
            return Promise.reject(new Error('Connector belongs to another environment'))
          }
          if (
            !removeIntent
          ...
          if (!policyAuthoritative) {
            ...
            return Promise.reject(new Error('Backup Policy state is not authoritative'))
          }
          if (backupPolicy.enabled && backupPolicy.connectorId === removing.id) {
            clearRemoveDispatchIntent()
            return Promise.reject(new Error('Disable the Backup Policy before removing this Connector'))
          }
          ...
          return store.removeConnector(removing.id, intent)
            .then((taskID) => {
              setRemoveIntent((current) => {
                if (
                  !current
...
function ConnectorCreateDrawer({
  ...
  onCreate: (connector: ConnectorCreateInput, intent: ConnectorMutationIntent) => Promise<void>
}) {
    ...
    >
      <DrawerContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Create an environment-owned S3-compatible destination. Credential references are preferred over direct values.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field id="connector-name" label="Name" className="sm:col-span-2">
            <Input
              ...
              autoFocus
            />
            {duplicate && <p className="text-xs text-destructive">This environment already has a connector with that name.</p>}
          </Field>
          <Field id="connector-endpoint" label="Endpoint" className="sm:col-span-2">
            <Input
          ...
          <Field id="connector-addressing" label="Addressing" className="sm:col-span-2">
            <Select
              ...
              options={[
                ...
                { value: 'virtual', label: 'Virtual hosted : bucket.endpoint/key' },
              ]}
            />
          </Field>
          <CredentialField
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              const intent = priorCreateIntent
              ...
              try {
                ...
                if (
                  ...
                  || currentCreateIntent.inputRevision !== createInputRevision.current
                ) return
                onOpenChange(false)
                reset()
              } catch (error) {
                ...
                if (
                  ...
                  if (!isNoResponseTransportUncertainty(error)) createIntentRef.current = null
                  setSubmitError(error instanceof Error ? error.message : 'Unable to create Connector')
                }
              } finally {
                setSubmitting(false)
              }
  ...
  return (
    <fieldset className="flex min-w-0 flex-col gap-2 rounded-lg border border-border bg-surface p-3">
      ...
      <Select
        id={`${id}-kind`}
        value={draft.kind}
        onValueChange={(kind) => onChange({ kind: kind as ConnectorCredentialInput['kind'], value: '' })}
        options={[
          { value: 'ref', label: 'Secret-store reference' },
          { value: 'value', label: 'Direct encrypted value' },
        ]}
      />
      <Label htmlFor={id}>{draft.kind === 'ref' ? 'Secret key name' : 'Credential value'}</Label>
      <Input
        id={id}
        ...
        placeholder={draft.kind === 'ref' ? 'R2_ACCESS_KEY_ID' : 'Stored encrypted at rest'}
        autoComplete="off"
      />
      {draft.kind === 'ref' && (
        <datalist id={listID}>
    ...
    <Dialog open={connector !== null} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>Environment-owned destination and redacted desired state.</DialogDescription>
        </DialogHeader>
        {connector && (
          <div className="flex flex-col gap-4">
            <dl className="grid gap-3 rounded-lg border border-border bg-surface p-3 text-sm sm:grid-cols-2">
              ...
              <Detail label="Addressing" value={connector.pathStyle ? 'path style' : 'virtual hosted style'} />
              <Detail label="Access key" value={credentialLabel(connector.credentials.accessKey)} />
              <Detail label="Secret key" value={credentialLabel(connector.credentials.secretKey)} />
            </dl>
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between gap-2">
                <span className="font-mono text-xs text-muted-foreground">connector.yaml</span>
                <CopyButton value={toYAML(connectorDocument(connector, tenantSlug, projectSlug, environmentName))} />
              </div>
              <pre className="overflow-x-auto rounded-lg border border-border bg-background p-4 font-mono text-xs leading-relaxed">
                {toYAML(connectorDocument(connector, tenantSlug, projectSlug, environmentName))}
              </pre>
            </div>
          </div>
        )}
...
function credentialLabel(credential: ConnectorCredential) {
  return credential.kind === 'ref' ? `secret ref : ${credential.name}` : 'direct value : encrypted'
}

function connectorDocument(connector: Connector, tenantSlug: string, projectSlug: string, environmentName: string) {
