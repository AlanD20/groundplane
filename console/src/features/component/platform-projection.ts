...
import type { ComponentResponse } from "./api";
...
function platformKind(value: string): PlatformComponentKind {
  ...
  throw new Error(
    `Controller returned unknown Platform Component kind ${value}`,
  );
}

export function platformComponentFromAPI(
  item: ComponentResponse,
): PlatformComponent {
  if (item.owner !== "platform" || item.owner_id) {
    throw new Error(
      "Controller returned a Component outside the Platform owner scope",
    );
  }
...
export function platformDNSFromComponent(
  item: ComponentResponse,
): PlatformInfra["dns"] {
  ...
  if (
    !("upstream_auto" in config) ||
    typeof config.upstream_auto !== "boolean" ||
    !("upstream_resolvers" in config) ||
    !Array.isArray(config.upstream_resolvers) ||
    config.upstream_resolvers.some((value) => typeof value !== "string") ||
    !("tailnet_delegation" in config) ||
    typeof config.tailnet_delegation !== "boolean" ||
    !("corefile_template" in config) ||
    typeof config.corefile_template !== "string" ||
    !("forwarders" in config) ||
    !Array.isArray(config.forwarders) ||
    config.forwarders.some(
      (value) =>
        typeof value.domain !== "string" ||
        !Array.isArray(value.resolvers) ||
        value.resolvers.some((resolver) => typeof resolver !== "string"),
    )
  ) {
    throw new Error(
      "Controller returned invalid CoreDNS Component configuration",
    );
  }
...
export function hydratePlatformComponents(
  items: ComponentResponse[],
): PlatformComponentHydration {
