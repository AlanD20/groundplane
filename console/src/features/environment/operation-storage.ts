...
export function environmentMutationKey(
  kind: EnvironmentMutationIntent["kind"],
  values: string[],
) {
...
const pendingResourceIntentStorageKey =
  "groundplane-pending-resource-removal-intents";
...
export function loadPendingResourceRemovals(): Map<
  string,
  PendingResourceRemoval
> {
  return new Map(
    readEntries(pendingResourceStorageKey).flatMap((entry) => {
...
export function persistPendingResourceRemovals(
  removals: Map<string, PendingResourceRemoval>,
) {
  writeEntries(pendingResourceStorageKey, [...removals]);
}

export function loadPendingResourceRemovalIntents(): Map<
  string,
  PendingResourceRemovalIntent
> {
  return new Map(
    readEntries(pendingResourceIntentStorageKey).flatMap((entry) => {
      if (!Array.isArray(entry) || entry.length !== 2) return [];
      ...
      if (
        !removal ||
        resourceRemovalKey(removal) !== key ||
...
export function persistPendingResourceRemovalIntents(
...
export function loadResourceRemovalRetryIntents(): Map<
  string,
  ResourceRemovalRetryIntent
> {
  return new Map(
    readEntries(retryIntentStorageKey).flatMap((entry) => {
      if (!Array.isArray(entry) || entry.length !== 2) return [];
      const [taskId, value] = entry as [unknown, unknown];
      if (typeof taskId !== "string" || !value || typeof value !== "object")
        return [];
      const candidate = value as {
        removal?: unknown;
        idempotencyKey?: unknown;
...
export function persistResourceRemovalRetryIntents(
  intents: Map<string, ResourceRemovalRetryIntent>,
) {
  writeEntries(retryIntentStorageKey, [...intents]);
}

export function loadEnvironmentDeletionFailures(): Map<
  string,
  EnvironmentDeletionFailure
> {
  return new Map(
    readEntries(deletionFailureStorageKey).flatMap((entry) => {
      if (!Array.isArray(entry) || entry.length !== 2) return [];
...
export function persistEnvironmentDeletionFailures(
  failures: Map<string, EnvironmentDeletionFailure>,
) {
  writeEntries(deletionFailureStorageKey, [...failures]);
}

function decodeMutationIntent(
  value: unknown,
): EnvironmentMutationIntent | undefined {
  if (!value || typeof value !== "object") return undefined;
  ...
  if (
    ...
    (typeof candidate.name !== "string" ||
      typeof candidate.networkPool !== "string")
  )
    return undefined;
...
export function loadEnvironmentMutationIntents(): Map<
  string,
  EnvironmentMutationIntent
> {
  return new Map(
    readEntries(mutationIntentStorageKey).flatMap((entry) => {
      if (!Array.isArray(entry) || entry.length !== 2) return [];
      const [key, value] = entry as [unknown, unknown];
      const intent = decodeMutationIntent(value);
      return typeof key === "string" && intent?.key === key
        ? [[key, intent]]
        : [];
    }),
  );
...
export function persistEnvironmentMutationIntents(
  intents: Map<string, EnvironmentMutationIntent>,
) {
