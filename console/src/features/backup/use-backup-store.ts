...
type BackupStoreOptions = {
  ...
  requestKeyRotation: (environmentId: string) => Promise<BackupKeyRotateResponse>
...
async function listBackupPolicyAttaches(request: VolumeRequest, environmentId: string) {
...
async function listBackupPolicyVolumes(request: VolumeRequest, environmentId: string) {
...
function requiredTaskId(response: { task_id?: string | null }, operation: string): string {
...
async function exportBackupKey(environmentId: string, signal?: AbortSignal): Promise<void> {
...
export function useBackupStore({
  ...
  const [backupPolicies, setBackupPolicies] = useState<Record<string, BackupPolicyState>>({})
  ...
  const policySaves = useRef(new Map<string, { fingerprint: string; promise: Promise<BackupPolicyDocument> }>())
  const policyReplayKeys = useRef(new Map<string, { fingerprint: string; key: string }>())

  const updatePolicy = useCallback((environmentId: string, update: (policy: BackupPolicyState) => void) => {
    if (!active.current) return
    setBackupPolicies((current) => {
      if (!active.current) return current
      const policy = structuredClone(current[environmentId] ?? emptyBackupPolicyState())
      update(policy)
  ...
  const loadBackupPolicy = useCallback(async (environmentId: string) => {
    ...
    const generation = (policyGenerations.current.get(environmentId) ?? 0) + 1
    ...
    const pending = (async () => {
      try {
        ...
        if (policyGenerations.current.get(environmentId) !== generation) return
        updatePolicy(environmentId, (policy) => {
    ...
    try {
      ...
      if (policyLoads.current.get(environmentId) === pending) policyLoads.current.delete(environmentId)
    }
  ...
  const loadRecoveryPoints = useCallback(async (environmentId: string, cursor?: string) => {
      ...
      try {
        ...
        if (pointGenerations.current.get(environmentId) === generation) {
          updatePolicy(environmentId, (policy) => {
            ...
            policy.recoveryPoints.loadError = error instanceof Error ? error.message : 'Unable to load Recovery Points'
    ...
    try {
      ...
      if (pointLoads.current.get(loadKey) === pending) pointLoads.current.delete(loadKey)
    }
  }, [request, updatePolicy])
  ...
  const replaceBackupPolicy = useCallback(async (
    ...
    policyGenerations.current.set(environmentId, (policyGenerations.current.get(environmentId) ?? 0) + 1)
    updatePolicy(environmentId, (policy) => {
      policy.saving = true
      policy.saveError = null
    })
    ...
    try {
      ...
      if (policySaves.current.get(environmentId)?.promise === pending) policySaves.current.delete(environmentId)
    }
  ...
  const getBackupPolicyState = useCallback(
    (environmentId: string) => backupPolicies[environmentId] ?? emptyBackupPolicyState(),
    [backupPolicies],
  )
